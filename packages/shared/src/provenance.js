 ...
 */
const input = (domain, table, column, labelUz, unit) => ({ source: 'input', domain, table, column, labelUz, unit });
const derived = (formula, dependsOn, labelUz, unit) => ({ source: 'derived', formula, dependsOn, labelUz, unit });
export const METRIC_PROVENANCE = {
    ...
    kwhLossTotal: derived('kwh_in − kwh_sold', ['kwhIn', 'kwhSold'], 'Jami yo‘qotish', 'kWh'),
    lossPct: derived('100 × (kwh_in − kwh_sold) / kwh_in', ['kwhIn', 'kwhSold'], 'Yo‘qotish darajasi', '%'),
    naturalLossPct: derived('100 × Σ kwh_loss_natural / Σ kwh_in', ['kwhLossNatural', 'kwhIn'], 'Tabiiy yo‘qotish ulushi', '%'),
    technicalLossPct: derived('100 × Σ kwh_loss_technical / Σ kwh_in', ['kwhLossTechnical', 'kwhIn'], 'Texnik yo‘qotish ulushi', '%'),
    illegalLossPct: derived('100 × Σ kwh_loss_illegal / Σ kwh_in', ['kwhLossIllegal', 'kwhIn'], 'Noqonuniy foydalanish ulushi', '%'),
    // ── Abonentlar ─────────────────────────────────────────────────────────────
    ...
    debtTotalMln: derived('debt_population_mln + debt_legal_mln + debt_budget_mln', ['debtPopulationMln', 'debtLegalMln', 'debtBudgetMln'], 'Jami qarzdorlik', 'mln so‘m'),
    ...
    tpOverloadedCount: derived('COUNT(*) FILTER (WHERE load_pct >= TP_OVERLOAD_PCT)', ['tpLoadPct'], 'Ortiqcha yuklangan TP lar', 'ta'),
    networkCapacityKva: derived('Σ ref.tp.rated_kva', ['tpRatedKva'], 'Texnik quvvat', 'kVA'),
    networkLoadKva: derived('Σ (rated_kva × load_pct / 100)', ['tpRatedKva', 'tpLoadPct'], 'Joriy foydalanish', 'kVA'),
    networkReserveKva: derived('Σ rated_kva − Σ (rated_kva × load_pct / 100)', ['tpRatedKva', 'tpLoadPct'], 'Zaxira quvvat', 'kVA'),
    distanceCompliancePct: derived('100 × COUNT(avg_distance_m ≤ TP_MAX_DISTANCE_M) / COUNT(*)', ['tpAvgDistanceM'], 'Masofa normasiga muvofiqlik', '%'),
    ...
    treeClearingKm: derived('Σ quantity WHERE work_type = TREE_CLEARING AND status = COMPLETED', ['workQuantity'], 'Daraxtlardan tozalangan tarmoq', 'km'),
    // ── Dalolatnomalar ─────────────────────────────────────────────────────────
    ...
    // ── Murakkab hosilalar ─────────────────────────────────────────────────────
    efficiencyIndex: derived('0.35×yo‘qotish + 0.20×qarzdorlik + 0.15×hisoblagich + 0.15×TP yuklama + 0.15×masofa', ['lossPct', 'debtTotalMln', 'metersOfflineCnt', 'tpLoadPct', 'tpAvgDistanceM'], 'Energiya samaradorlik indeksi', 'ball'),
    technicalLossGapPp: derived('amaldagi texnik yo‘qotish % − TECHNICAL_LOSS_PCT normasi', ['technicalLossPct', 'technicalLossStandard'], 'Standartdan farq', 'p.p.'),
    debtMonths: derived('debt_total_mln / (oylik hisoblangan summa)', ['debtTotalMln', 'kwhSold'], 'Qarzdorlik (oylarda)', 'oy'),
};
...
export function inputMetrics() {
    return Object.keys(METRIC_PROVENANCE).filter((k) => METRIC_PROVENANCE[k].source === 'input');
}
/** Berilgan domenda kiritiladigan metrikalar. */
export function metricsForDomain(domain) {
    return Object.keys(METRIC_PROVENANCE).filter((k) => {
