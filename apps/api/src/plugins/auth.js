...
export async function loadUser(userId) {
    const u = await queryOne(`SELECT id, login, full_name, role FROM sec.app_user WHERE id = $1 AND is_active`, [userId]);
    if (!u)
        return null;
    const scopes = await query(`SELECT scope_type, scope_id FROM sec.user_scope WHERE user_id = $1`, [userId]);
    ...
    if (elektrosetIds.length > 0) {
        const expanded = await query(`SELECT id FROM ref.mfy WHERE elektroset_id = ANY($1::int[]) AND valid_to IS NULL`, [elektrosetIds]);
        mfyIds = [...new Set([...mfyIds, ...expanded.map((r) => r.id)])];
    }
    // Admin barcha MFY larga yozadi.
    if (u.role === 'admin') {
        const all = await query(`SELECT id FROM ref.mfy WHERE valid_to IS NULL`);
        mfyIds = all.map((r) => r.id);
    }
    return { id: u.id, login: u.login, fullName: u.full_name, role: u.role, mfyIds, elektrosetIds };
}
export async function login(loginName, password, ip, ua) {
    const row = await queryOne(`SELECT id, password_hash, is_active FROM sec.app_user WHERE login = $1`, [loginName]);
    ...
    await withTransaction({ userId: user.id, role: user.role, mfyIds: [], requestId: 'login' }, async (client) => {
        await client.query(`INSERT INTO sec.refresh_token
           (user_id, family_id, token_hash, expires_at, client_ip, user_agent)
         VALUES ($1, $2, $3, now() + ($4 || ' days')::interval, $5, $6)`, [user.id, randomUUID(), hashToken(refreshToken), config.auth.refreshTtlDays, ip ?? null, ua ?? null]);
...
export async function rotateRefresh(token) {
    const row = await queryOne(`SELECT id, user_id, family_id, used_at::text, revoked_at::text,
    ...
    if (row.used_at) {
        await withTransaction({ userId: row.user_id, role: 'system', mfyIds: [], requestId: 'token-reuse' }, async (client) => {
            await client.query(`UPDATE sec.refresh_token SET revoked_at = now()
    ...
    await withTransaction({ userId: user.id, role: user.role, mfyIds: [], requestId: 'refresh' }, async (client) => {
        await client.query('UPDATE sec.refresh_token SET used_at = now() WHERE id = $1', [row.id]);
        await client.query(`INSERT INTO sec.refresh_token (user_id, family_id, token_hash, expires_at)
         VALUES ($1, $2, $3, now() + ($4 || ' days')::interval)`, [user.id, row.family_id, hashToken(next), config.auth.refreshTtlDays]);
    });
    return { user, accessToken: await signAccessToken(user), refreshToken: next };
}
export async function revokeRefresh(token) {
    await withTransaction({ userId: null, role: 'system', mfyIds: [], requestId: 'logout' }, async (client) => {
        await client.query('UPDATE sec.refresh_token SET revoked_at = now() WHERE token_hash = $1', [hashToken(token)]);
    });
}
...
const authPlugin = async (app) => {
    ...
    app.decorate('assertMfyWrite', (req, mfyId) => {
        if (!req.user)
            return false;
