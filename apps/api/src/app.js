...
export async function buildApp() {
    ...
    if (!config.isProd) {
        const LOCAL_ORIGIN = /^https?:\/\/(?:localhost|\[::1\]|127\.\d{1,3}\.\d{1,3}\.\d{1,3}|10\.\d{1,3}\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|172\.(?:1[6-9]|2\d|3[01])\.\d{1,3}\.\d{1,3})(?::\d{1,5})?$/;
    ...
    app.get('/api/health/db', async () => {
        const row = await queryOne('SELECT version() AS v');
