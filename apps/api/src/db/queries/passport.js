...
function buildRows(data, isDistrict) {
...
export async function mfyPassport(ctx, mfyId, period) {
    const row = await queryOne(`SELECT * FROM agg.v_mfy_passport WHERE mfy_id = $1 AND period_month = ($2 || '-01')::date`, [mfyId, period], ctx);
    if (!row)
        return null;
...
export async function tumanPassport(ctx, period) {
    const row = await queryOne(`SELECT * FROM agg.v_tuman_passport WHERE period_month = ($1 || '-01')::date`, [period], ctx);
    if (!row)
        return null;
...
async function getSnapshotMeta(ctx, scopeType, scopeId, period) {
    const s = await queryOne(`SELECT ps.frozen_at::text AS at, u.full_name AS by, ps.content_sha256 AS sha
     ...
     WHERE ps.scope_type = $1 AND ps.scope_id IS NOT DISTINCT FROM $2
       AND ps.period_month = ($3 || '-01')::date`, [scopeType, scopeId, period], ctx);
 ...
 */
export async function reconcile(ctx, period) {
...
export async function freeze(ctx, scopeType, scopeId, period) {
    const passport = scopeType === 'TUMAN'
        ? await tumanPassport(ctx, period)
        : await mfyPassport(ctx, scopeId, period);
    if (!passport)
        throw new Error(`${period} davri uchun pasport ma'lumoti topilmadi`);
    if (passport.frozen) {
        const existing = await queryOne(`SELECT id FROM fact.passport_snapshot
       WHERE scope_type = $1 AND scope_id IS NOT DISTINCT FROM $2
         AND period_month = ($3 || '-01')::date`, [scopeType, scopeId, period], ctx);
    ...
    return withTransaction(ctx, async (client) => {
        const { rows } = await client.query(`INSERT INTO fact.passport_snapshot
         (scope_type, scope_id, period_month, payload, content_sha256, frozen_by)
       VALUES ($1, $2, ($3 || '-01')::date, $4, $5, $6)
       RETURNING id`, [scopeType, scopeId, period, JSON.stringify(payload), sha256, ctx.userId]);
        return { id: rows[0].id, sha256, alreadyFrozen: false };
    });
}
export async function listSnapshots(ctx, scopeType) {
    const rows = await query(`SELECT ps.id, ps.scope_type, ps.scope_id,
