...
export async function bootstrap(ctx) {
    const [elektrosets, mfys, norms, categories, refresh, range] = await Promise.all([
        query(`SELECT id, code, name_uz, name_uz_cyr FROM ref.elektroset WHERE is_active ORDER BY id`, [], ctx),
        query(`SELECT m.id, m.elektroset_id, e.name_uz AS elektroset_name, m.code, m.name_uz,
       ...
       WHERE m.valid_to IS NULL ORDER BY m.sort_order, m.name_uz`, [], ctx),
        query(`SELECT id, scope_type, scope_id, metric, value_num, unit,
              effective_from::text, effective_to::text, source_doc
       FROM ref.norm
       WHERE effective_to IS NULL OR effective_to > CURRENT_DATE
       ORDER BY metric, scope_type`, [], ctx),
        query(`SELECT code, name_uz FROM ref.consumer_category ORDER BY id`, [], ctx),
        queryOne(`SELECT max(finished_at)::text AS at FROM agg.refresh_log WHERE ok`, [], ctx),
        dataRange(ctx),
    ]);
    return {
        ...
        mfys: mfys.map((m) => ({
            ...
            nameUz: String(m['name_uz']), nameUzCyr: m['name_uz_cyr'] ?? null,
        ...
        norms: norms.map((n) => ({
            id: Number(n['id']), scopeType: n['scope_type'],
            ...
            effectiveTo: n['effective_to'] ?? null,
            sourceDoc: n['source_doc'] ?? null,
        })),
        categories: categories.map((c) => ({
            code: c.code, nameUz: c.name_uz,
        })),
        lastRefreshAt: refresh?.at ?? null,
        dataRange: range,
    };
}
export async function listTp(ctx, mfyId) {
    const rows = await query(`SELECT t.id, t.mfy_id, m.name_uz AS mfy_name, t.code, t.name, t.rated_kva,
    ...
    return rows.map((r) => ({
        ...
        code: String(r['code']), name: r['name'] ?? null,
        ...
        commissionedOn: r['commissioned_on'] ?? null,
        decommissionedOn: r['decommissioned_on'] ?? null,
    }));
}
export async function listNetworkSegments(ctx, mfyId) {
    const rows = await query(`SELECT id, mfy_id, voltage_kv, line_type, length_km,
    ...
    return rows.map((r) => ({
        ...
        lineType: r['line_type'], lengthKm: Number(r['length_km']),
        installedOn: r['installed_on'] ?? null,
        retiredOn: r['retired_on'] ?? null,
    }));
}
export async function getMfy(ctx, id) {
    const r = await queryOne(`SELECT m.id, m.elektroset_id, e.name_uz AS elektroset_name, m.code, m.name_uz,
            m.name_uz_cyr, m.short_name, m.sort_order, m.grid_row, m.grid_col
     FROM ref.mfy m JOIN ref.elektroset e ON e.id = m.elektroset_id
     WHERE m.id = $1`, [id], ctx);
    if (!r)
    ...
    return {
        ...
        nameUz: String(r['name_uz']), nameUzCyr: r['name_uz_cyr'] ?? null,
