...
export const SYSTEM_CONTEXT: AppContext = {
  userId: null,
  role: 'system',
  mfyIds: [],
  requestId: 'system',
...
export async function queryOne<T extends pg.QueryResultRow = pg.QueryResultRow>(
  ...
  const rows = await query<T>(text, params, ctx);
  return rows[0] ?? null;
}
...
export function isPgError(err: unknown): err is Error & {
  code?: string;
  constraint?: string;
  detail?: string;
  hint?: string;
