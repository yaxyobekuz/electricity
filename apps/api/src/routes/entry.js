 ...
 */
import { DOMAINS, createSubmissionSchema, energyBalancePatchSchema, monthlyReturnSchema, reviewActionSchema, } from '@beap/shared';
import { z } from 'zod';
...
const entryRoutes = async (app) => {
    ...
    app.get('/review-queue', { onRequest: [app.requireRole('elektroset_manager', 'admin')] }, async (req) => e.reviewQueue(req.ctx));
    ...
    app.get('/submission/:id', async (req, reply) => {
        ...
        if (!sub)
            return reply.code(404).send({ error: 'not_found', message: 'Konvert topilmadi' });
    ...
    app.post('/submission/:id/submit', async (req, reply) => {
        ...
        if (!validation.ok) {
            return reply.code(422).send({
                ...
                errors: Object.fromEntries(validation.issues.filter((i) => i.severity === 'error').map((i) => [i.path, i.message])),
            });
        }
        return e.changeStatus(req.ctx, id, 'submitted', null);
    });
    app.post('/submission/:id/approve', { onRequest: [app.requireRole('elektroset_manager', 'admin')] }, async (req) => {
        const { id } = idParam.parse(req.params);
    ...
    app.post('/submission/:id/reject', { onRequest: [app.requireRole('elektroset_manager', 'admin')] }, async (req, reply) => {
        const { id } = idParam.parse(req.params);
        const body = reviewActionSchema.parse(req.body ?? {});
        if (!body.note) {
    ...
    app.get('/domains', async () => DOMAINS.map((d) => d));
};
