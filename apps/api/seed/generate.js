...
export function allocateInt(total, weights, min = 0) {
...
export function allocateFloat(total, weights, decimals = 1) {
 ...
 */
function allocateWithPins(total, items, kind, decimals = 1) {
...
function lastCompletePeriod(today = new Date()) {
...
function buildColdSnaps(from, to) {
    const map = new Map();
...
async function setAuditTriggers(client, enabled) {
  ...
  `);
    for (const r of rows) {
        await client.query(`ALTER TABLE ${r.sch}.${r.tbl} ${enabled ? 'ENABLE' : 'DISABLE'} TRIGGER zz_audit`);
    }
}
export async function generateSeed(client, opts) {
    const { log } = opts;
    ...
    for (let i = opts.months - 1; i >= 0; i -= 1)
        periods.push(addMonths(lastPeriod, -i));
  ...
  `);
    ...
    for (const u of userDefs) {
        const { rows } = await client.query(`INSERT INTO sec.app_user (login, full_name, password_hash, role)
       VALUES ($1, $2, $3, $4) RETURNING id`, [u.login, u.fullName, hash, u.role]);
    ...
    // ── 2. MFY lar ───────────────────────────────────────────────────────────
    log('MFY lar…');
    const { rows: esRows } = await client.query('SELECT id, code FROM ref.elektroset');
    const esByCode = new Map(esRows.map((r) => [r.code, r.id]));
    const mfyIds = [];
    for (const [idx, m] of cfg.mfys.entries()) {
        const { rows } = await client.query(`INSERT INTO ref.mfy (elektroset_id, code, name_uz, name_uz_cyr, short_name,
    ...
    const profileOf = (m) => cfg.profiles[m.profile] ?? cfg.profiles['normal'];
    ...
    const popAlloc = allocateWithPins(dt['consumersPopulation'], cfg.mfys.map((m) => ({ weight: m.weight, pin: m.pinned?.consumersPopulation })), 'int');
    const legalAlloc = allocateWithPins(dt['consumersLegal'], cfg.mfys.map((m) => ({ weight: m.weight, pin: m.pinned?.consumersLegal })), 'int');
    const tpAlloc = allocateWithPins(dt['tpCount'], cfg.mfys.map((m) => ({ weight: m.weight, pin: m.pinned?.tpCount })), 'int');
    const line04Alloc = allocateWithPins(dt['lineKm04'], cfg.mfys.map((m) => ({ weight: m.weight, pin: m.pinned?.lineKm04 })), 'float', 2);
    const line10Alloc = allocateWithPins(dt['lineKm10'], cfg.mfys.map((m) => ({ weight: m.weight, pin: m.pinned?.lineKm10 })), 'float', 2);
    ...
    if (statedTotal !== undefined && statedTotal !== actualTotal) {
        warnings.push(`Pasportda jami iste'molchilar ${statedTotal} deb yozilgan, bo'laklar yig'indisi ${actualTotal}. ` +
            `Tizim bo'laklardan hisoblaydi (farq: ${statedTotal - actualTotal}).`);
    }
    warnings.push(`Tuman 4-qatori: 10 kV tarmoq ${dt['lineKm10']} km deb olindi ` +
    ...
    await bulkInsert(client, 'ref.tp', ['mfy_id', 'code', 'name', 'rated_kva', 'voltage_class', 'avg_distance_m', 'commissioned_on'], tpRows);
    rowCounts['ref.tp'] = tpRows.length;
    const { rows: tpFetched } = await client.query(`SELECT id, mfy_id, rated_kva, avg_distance_m, voltage_class FROM ref.tp
    ...
    await bulkInsert(client, 'ref.network_segment', ['mfy_id', 'voltage_kv', 'line_type', 'length_km', 'installed_on'], segRows);
    ...
    function districtLoss(monthIdx) {
    ...
    const subKey = (k) => `${k.mfyIdx}|${k.domain}|${k.period}`;
    async function createSubmissions(period, status, onlyMfyIdx) {
        ...
        for (const [idx, m] of cfg.mfys.entries()) {
            if (onlyMfyIdx !== undefined && idx !== onlyMfyIdx)
                continue;
            const operator = userIds.get(idx === 0 ? 'operator1' : idx === 2 ? 'operator2' : 'operator3') ?? adminId;
        ...
        for (let s = 0; s < rows.length; s += 200) {
            ...
            const { rows: inserted } = await client.query(`INSERT INTO fact.submission (${cols.join(', ')}) VALUES ${tuples.join(', ')} RETURNING id`, params);
    ...
    for (const [monthIdx, period] of periods.entries()) {
        ...
        const debtWeights = cfg.mfys.map((m, i) => {
            ...
            const factor = m.profile === 'problem-debt'
        ...
        const offlineAlloc = allocateWithPins(Math.round(dt['metersOfflineCnt'] * repairTrend), cfg.mfys.map((m, i) => ({
            weight: consumerShare[i],
            pin: monthIdx === totalMonths - 1 ? m.pinned?.metersOfflineCnt : undefined,
        })), 'int');
        const lowAlloc = allocateWithPins(dt['lowConsumptionCnt'], cfg.mfys.map((m, i) => ({
            weight: consumerShare[i],
            pin: monthIdx === totalMonths - 1 ? m.pinned?.lowConsumptionCnt : undefined,
        })), 'int');
        const replaceNeedAlloc = allocateWithPins(Math.round(dt['metersReplaceNeedCnt'] * repairTrend), cfg.mfys.map((m, i) => ({
            weight: consumerShare[i],
            pin: monthIdx === totalMonths - 1 ? m.pinned?.metersReplaceNeedCnt : undefined,
        })), 'int');
        const replacedAlloc = allocateWithPins(Math.round(dt['metersReplacedCnt'] * repairTrend), cfg.mfys.map((m, i) => ({
            weight: consumerShare[i],
        ...
        const repair04Alloc = allocateWithPins(dt['repairKm04'] * repairTrend, cfg.mfys.map((m) => ({ weight: m.weight, pin: isLast ? m.pinned?.repairKm04 : undefined })), 'float', 2);
        const repair10Alloc = allocateWithPins(dt['repairKm10'] * repairTrend, cfg.mfys.map((m) => ({ weight: m.weight, pin: isLast ? m.pinned?.repairKm10 : undefined })), 'float', 2);
        const tpRepairAlloc = allocateInt(Math.round(dt['tpRepairNeeded'] * repairTrend), tpAlloc.map((n) => n));
        for (const [idx, m] of cfg.mfys.entries()) {
            const prof = profileOf(m);
            ...
            const winterSpike = m.profile === 'problem-illegal' && (mon >= 11 || mon <= 2) ? 1.35 : 1.0;
            let monthIn = 0;
            let monthLoss = 0;
            for (let day = 1; day <= nDays; day += 1) {
                ...
                if (outageDays.get(m.code)?.has(dstr))
                    kwhIn *= 0.7;
            ...
            mfyTps.forEach((t, k) => {
                const load = clamp(between(38, 74) + overloadBias + gauss(0, 7) + (mon >= 11 || mon <= 2 ? 8 : 0), 5, 128);
                const condition = load >= 90 ? 'OVERLOAD' : load >= 78 ? 'ATTENTION' : rand() < 0.02 ? 'FAULT' : 'GOOD';
    ...
    await bulkInsert(client, 'fact.energy_balance_daily', ['submission_id', 'mfy_id', 'biz_date', 'kwh_in', 'kwh_sold',
        'kwh_loss_natural', 'kwh_loss_technical', 'kwh_loss_illegal'], ebRows);
    rowCounts['fact.energy_balance_daily'] = ebRows.length;
    await bulkInsert(client, 'fact.mfy_monthly_return', ['submission_id', 'mfy_id', 'period_month', 'consumers_population', 'consumers_legal',
        'consumers_active', 'consumers_disconnected', 'consumers_new',
        'debt_population_mln', 'debt_legal_mln', 'debt_budget_mln',
        'meters_offline_cnt', 'low_consumption_cnt', 'meters_replace_need_cnt', 'meters_replaced_cnt'], mrRows);
    rowCounts['fact.mfy_monthly_return'] = mrRows.length;
    await bulkInsert(client, 'fact.tp_status_monthly', ['submission_id', 'tp_id', 'period_month', 'load_pct', 'peak_kva',
        'condition', 'under_load', 'repair_needed', 'repair_reason'], tsRows);
    rowCounts['fact.tp_status_monthly'] = tsRows.length;
    await bulkInsert(client, 'fact.network_defect', ['submission_id', 'mfy_id', 'period_month', 'voltage_kv', 'repair_needed_km', 'repaired_km'], ndRows);
    rowCounts['fact.network_defect'] = ndRows.length;
    await bulkInsert(client, 'fact.debt_top_entry', ['submission_id', 'mfy_id', 'period_month', 'rank', 'debtor_name', 'category', 'amount_mln'], dtRows);
    ...
    await bulkInsert(client, 'fact.tp_reading_daily', ['submission_id', 'tp_id', 'biz_date', 'max_load_kw', 'min_load_kw',
        'avg_voltage_v', 'outage_count', 'outage_minutes'], trRows, 600);
    ...
    const treeAlloc = allocateWithPins(dt['treeClearingKmYear'], cfg.mfys.map((m) => ({ weight: m.weight, pin: m.pinned?.treeClearingKmYear })), 'float', 1);
    for (const [idx, m] of cfg.mfys.entries()) {
    ...
    await bulkInsert(client, 'fact.work', ['mfy_id', 'tp_id', 'work_type', 'title_uz', 'description', 'status',
    ...
    let actNo = 1;
    for (const [idx, m] of cfg.mfys.entries()) {
        const intensity = m.profile === 'problem-illegal' ? 6 : m.profile === 'improving' ? 4 : rand() < 0.5 ? 2 : 1;
    ...
    await bulkInsert(client, 'fact.violation_act', ['mfy_id', 'tp_id', 'act_no', 'act_date', 'consumer_ref',
        'kwh_identified', 'fine_mln', 'status'], vaRows);
    ...
    if (lateIdx >= 0)
        await createSubmissions(currentPeriod, 'draft', lateIdx);
    // Bittasi ko'rib chiqishga yuborilgan — review navbati bo'sh bo'lmasin
    if (reviewIdx >= 0)
        await createSubmissions(currentPeriod, 'submitted', reviewIdx);
    // Yuborilgan konvertga haqiqiy ma'lumot ham qo'shamiz
    if (reviewIdx >= 0) {
        const sub = submissionIds.get(subKey({ mfyIdx: reviewIdx, domain: 'ENERGY_BALANCE', period: currentPeriod }));
        if (sub) {
            const rows = [];
            ...
            await bulkInsert(client, 'fact.energy_balance_daily', ['submission_id', 'mfy_id', 'biz_date', 'kwh_in', 'kwh_sold',
                'kwh_loss_natural', 'kwh_loss_technical', 'kwh_loss_illegal'], rows);
    ...
    if (goravonIdx >= 0) {
        const testSub = submissionIds.get(subKey({ mfyIdx: goravonIdx, domain: 'MONTHLY_RETURN', period: lastPeriod }));
        try {
            await client.query('SAVEPOINT integrity_demo');
            await client.query(`INSERT INTO fact.mfy_monthly_return
         ...
         VALUES ($1, $2, $3, 820, 9, 810, $4, $5, 0)`, [testSub, mfyIds[goravonIdx], `${lastPeriod}-01`,
