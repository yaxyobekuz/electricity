...
const close = (a: number, b: number, tolPct = 0.1): boolean =>
  Math.abs(a - b) <= Math.max(0.05, (Math.abs(b) * tolPct) / 100);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const selects = ROLLUP_COLUMNS.map(
      (c) => `round(sum(p.${c})::numeric, 2) AS mfy_${c}, round(max(t.${c})::numeric, 2) AS tuman_${c}`,
    ).join(',\n           ');
    ...
    for (const row of rollup as Record<string, unknown>[]) {
    ...
    add(
      `Roll-up: SUM(MFY) = TUMAN (${rollup.length} oy × ${ROLLUP_COLUMNS.length} qator)`,
      mismatches === 0,
      mismatches === 0 ? `${rollup.length * ROLLUP_COLUMNS.length} ta tekshiruv` : mismatchDetail.join('; '),
    );
    ...
    const { rows: dupRows } = await client.query<{ n: string }>(`
    ...
    `);
    add(
      'Bir davr + domen uchun bitta tasdiqlangan konvert',
      Number(dupRows[0]?.n ?? 0) === 0,
      `${dupRows[0]?.n ?? 0} ta takror`,
    );
