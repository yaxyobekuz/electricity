...
async function run(client: pg.PoolClient, ctx: Ctx, c: Case): Promise<void> {
...
const CASES: Case[] = [
    ...
    run: (c, x) =>
      c.query(
        `INSERT INTO fact.mfy_monthly_return
         ...
         VALUES ($1, $2, ($3 || '-01')::date, 100, 10, 500)`,
        [x.submissionId, x.mfyId2, x.period],
      ),
  },
  {
    ...
    run: (c, x) =>
      c.query(
        `INSERT INTO fact.mfy_monthly_return
         ...
         VALUES ($1, $2, ($3 || '-01')::date, 100, 10, 100, 5, 20)`,
        [x.submissionId, x.mfyId2, x.period],
      ),
  },
  {
    ...
    run: (c, x) =>
      c.query(
        `INSERT INTO fact.mfy_monthly_return
         ...
         VALUES ($1, $2, ($3 || '-01')::date, 820, 9, 810, 2449.1, 1816.5)`,
        [x.submissionId, x.mfyId2, x.period],
      ),
  },
  {
    ...
    run: (c, x) =>
      c.query(
        `INSERT INTO fact.tp_status_monthly
         ...
         VALUES ($1, $2, ($3 || '-01')::date, 350)`,
        [x.submissionId, x.tpId, x.period],
      ),
  },
  {
    ...
    run: (c, x) =>
      c.query(
        `INSERT INTO fact.tp_status_monthly
         ...
         VALUES ($1, $2, ($3 || '-01')::date, 50, true, NULL)`,
        [x.submissionId, x.tpId, x.period],
      ),
  },
  {
    ...
    run: (c, x) =>
      c.query(
        `INSERT INTO fact.tp_reading_daily
         ...
         VALUES ($1, $2, CURRENT_DATE - 405, 10, 90)`,
        [x.submissionId, x.tpId],
      ),
  },
  {
    ...
    run: (c, x) =>
      c.query(
        `INSERT INTO fact.work
         ...
         VALUES ($1, 'OTHER', 'Test ishi', 'COMPLETED', 40, NULL)`,
        [x.mfyId],
      ),
  },
  {
  ...
  },
  {
    ...
    run: async (c, x) => {
      ...
      const check = await c.query<{ content_sha256: string }>(
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const ref = await client.query<{
      mfy_id: number; mfy_id2: number; tp_id: number; user_id: number;
      submission_id: number; period: string;
    }>(`
      SELECT
    ...
    for (const c of CASES) await run(client, ctx, c);
