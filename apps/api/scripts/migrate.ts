...
async function resetDatabase(client: pg.PoolClient): Promise<void> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (reset) {
      if (config.isProd) throw new Error('--reset ishlab chiqarishda taqiqlanadi');
    ...
    const res = await client.query<{ filename: string; sha256: string }>(
      'SELECT filename, sha256 FROM sec.schema_migration',
    );
    for (const r of res.rows) applied.set(r.filename, r.sha256);
    ...
    for (const file of files) {
      ...
      try {
        ...
        await client.query(
          'INSERT INTO sec.schema_migration (filename, sha256, duration_ms) VALUES ($1, $2, $3)',
          [file.filename, file.hash, ms],
        );
