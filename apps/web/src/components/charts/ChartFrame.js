...
export function ChartFrame({ title, subtitle, legend, height = 240, children, tableData, tableColumns, csvName = 'export', actions, className, }) {
    ...
    const legendList = legend && legend.length > 1 && view === 'chart' ? (<ul className="chart-frame__legend">
        {legend.map((item) => (<li key={item.label} className="flex items-center gap-1.5">
            <span className="chart-frame__swatch" style={{ background: item.color }} aria-hidden="true"/>
     ...
     */
    const legendInHeader = !title && !subtitle;
    return (<div className={cn('flex min-w-0 flex-col gap-3', className)}>
      {(title || legend || hasTable || actions) && (<div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            {actions}
            {hasTable && (<>
                <ToggleButtonGroup aria-label="Ko‘rinish" selectedKeys={new Set([view])} selectionMode="single" size="sm" onSelectionChange={(keys) => {
                    const next = [...keys][0];
                ...
                <Button isIconOnly aria-label="CSV yuklab olish" size="sm" variant="ghost" onPress={downloadCsv}>
      ...
      {view === 'chart' ? (<div style={{ height }} className="min-w-0">
          {children}
        </div>) : (<div className="scroll-y max-h-80 rounded-lg border border-border/70">
          <table className="dt">
            <thead>
              <tr>
                {tableColumns.map((c) => (<th key={c.key} className={c.align === 'right' ? 'text-right' : undefined}>
                    {c.label}
                  </th>))}
              </tr>
            </thead>
            <tbody>
              {tableData.map((row, i) => (<tr key={i}>
                  {tableColumns.map((c) => (<td key={c.key} className={c.align === 'right' ? 'num' : undefined}>
                      {c.render(row)}
                    </td>))}
                </tr>))}
            </tbody>
