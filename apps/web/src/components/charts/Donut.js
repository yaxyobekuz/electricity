...
export function Donut({ slices, centerValue, centerLabel, height = 220, csvName = 'donut', title, maxColors = 3, formatValue, legendSide = false, }) {
    ...
    const fmt = formatValue ?? ((v) => v.toLocaleString('uz-Latn-UZ'));
    const columns = [
    ...
    const sideLegend = legendSide ? (<ul className="flex min-w-0 flex-1 flex-col justify-center gap-2.5">
      {prepared.map((s) => (<li key={s.id} className="flex items-start gap-2">
          <span aria-hidden="true" className="mt-1 inline-block size-2 shrink-0 rounded-full" style={{ background: s.color }}/>
          <div className="min-w-0">
            <p className="truncate text-[11px] leading-tight text-muted">{s.label}</p>
            <p className="text-[12.5px] font-semibold leading-tight" style={{ fontVariantNumeric: 'tabular-nums' }}>
              {s.display ?? fmt(s.value)}
              {total > 0 && (<span className="ml-1 font-normal text-muted">
    ...
    return (<ChartFrame csvName={csvName} height={height} legend={legendSide ? undefined : prepared.map((s) => ({ label: s.label, color: s.color }))} tableColumns={columns} tableData={prepared} title={title}>
      <div className={legendSide ? 'flex h-full items-center gap-3' : 'relative h-full'}>
        ...
        <ResponsivePie activeOuterRadiusOffset={5} arcLabel={() => ''} borderColor={t.surface} 
    /* 2px surface rangli bo'shliq — kontur EMAS */
    borderWidth={2} colors={{ datum: 'data.color' }} cornerRadius={2} data={prepared.map((s) => ({
            id: s.id, label: s.label, value: s.value, color: s.color,
        }))} enableArcLabels={false} enableArcLinkLabels={false} innerRadius={0.68} margin={{ top: 6, right: 6, bottom: 6, left: 6 }} padAngle={0.6} theme={nivoTheme(t)} tooltip={({ datum }) => (<div className="chart-tooltip">
              ...
              {total > 0 && (<div className="chart-tooltip__row">
          ...
          {(centerValue || centerLabel) && (<div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
              {centerValue && (<span className="tabular text-lg font-bold leading-none">{centerValue}</span>)}
              {centerLabel && (<span className="mt-1 max-w-[80%] text-center text-[10px] leading-tight text-muted">
                  {centerLabel}
                </span>)}
            </div>)}
        </div>
