...
export function DivergingBar({ rows, height = 300, onSelect }) {
    ...
    return (<ChartFrame csvName="texnik-yoqotish" height={height} tableColumns={columns} tableData={rows} legend={[
      ...
      <ResponsiveBar animate={false} axisBottom={{
            tickSize: 0,
            tickPadding: 6,
            format: (v) => `${v > 0 ? '+' : ''}${v}`,
        }} axisLeft={{ tickSize: 0, tickPadding: 8 }} borderRadius={3} colors={(d) => d.data.color} data={data} enableGridX enableGridY={false} indexBy="mfy" keys={['gap']} labelSkipWidth={9999} layout="horizontal" margin={{ top: 4, right: 16, bottom: 26, left: 104 }} markers={[
            {
                axis: 'x',
                value: 0,
        ...
        ]} padding={0.28} theme={nivoTheme(t)} 
    /*
     * min/max — MAJBURIY.
     *
     ...
     */
    valueScale={{ type: 'linear', min: 'auto', max: 'auto' }} onClick={(d) => onSelect?.(d.data.mfyId)} tooltip={({ data }) => {
