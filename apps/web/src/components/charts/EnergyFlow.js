...
export function EnergyFlow({ nodes, height = 260 }) {
    ...
    if (!data) {
        return (<div className="flex h-40 items-center justify-center text-sm text-muted">
        Ma’lumot yo‘q
      </div>);
    }
    return (<ChartFrame csvName="energiya-balansi" height={height} legend={nodes
            .filter((n) => n.key !== 'in')
            .map((n) => ({ label: n.labelUz, color: colorByKey[n.key] ?? t.muted }))} tableColumns={columns} tableData={nodes}>
      <ResponsiveSankey data={data} align="justify" colors={(node) => node.nodeColor ?? t.series[0]} enableLinkGradient label={(node) => String(node.id)} labelPadding={10} labelPosition="outside" labelTextColor={t.ink2} linkBlendMode="normal" linkContract={2} linkOpacity={0.42} linkHoverOpacity={0.7} margin={{ top: 8, right: 168, bottom: 8, left: 130 }} nodeBorderWidth={0} nodeOpacity={1} nodeSpacing={16} nodeThickness={16} theme={nivoTheme(t)} nodeTooltip={({ node }) => (<div className="chart-tooltip">
          ...
          </div>)} linkTooltip={({ link }) => (<div className="chart-tooltip">
 ...
 */
export function EnergyBalanceBars({ nodes }) {
    const t = useVizTokens();
    const source = nodes.find((n) => n.key === 'in');
    if (!source)
        return null;
    ...
    return (<ul className="flex flex-col gap-2.5">
      {nodes.map((n) => {
            ...
            return (<li key={n.key} className="flex flex-col gap-1">
            <div className="flex items-baseline justify-between gap-2 text-xs">
              <span className={isSource ? 'font-semibold' : 'text-muted'}>{n.labelUz}</span>
              <span className="tabular shrink-0 font-medium">
                {energy(n.kwh).text}
                {!isSource && (<span className="ml-1.5 text-[11px] text-muted">({pct(n.pct, 1)})</span>)}
              </span>
            </div>
            <div className="loadbar">
              <span className="loadbar__fill" style={{
                    width: `${width}%`,
