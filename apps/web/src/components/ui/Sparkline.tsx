...
export function Sparkline({
  values, color, accent, width = 120, height = 32, fill = true, className,
  variant = 'line',
}: SparklineProps) {
  return variant === 'bars' ? (
    <SparkBars className={className} color={color} height={height} values={values} width={width} />
  ) : (
    <SparkLine
      accent={accent}
      className={className}
...
function SparkBars({
  ...
  return (
    <svg
      aria-label="Oxirgi kunlar dinamikasi"
      className={className}
      height={height}
      role="img"
