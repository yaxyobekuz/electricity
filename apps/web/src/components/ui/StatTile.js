import { energyParts, isKnownMetric, moneyParts, num, pct, periodLabel, provenanceText, } from '@beap/shared';
...
const formatValue = (tile) => fmtByUnit(tile.value, tile.unit);
/**
 ...
 */
function changeText(tile) {
    if (tile.value === null || tile.prevValue === null)
        return null;
...
const sparkCaption = (t) => t.spark.length < 2 ? '' : t.sparkBucket === 'day' ? `${t.spark.length} kun` : `${t.spark.length} oy`;
export function StatTile({ tile, icon, tone = 'blue', compact }) {
    ...
    const provenance = isKnownMetric(tile.metric) ? provenanceText(tile.metric) : null;
    ...
    return (<article className={cn('kpi group', `tone-${tone}`, compact && 'gap-2 p-3')}>
      <header className="flex items-start justify-between gap-2">
        <h3 className="kpi__label pr-1">{tile.labelUz}</h3>

        <div className="flex shrink-0 items-center gap-1">
          {provenance && (<Popover>
              <Button isIconOnly aria-label="Ma’lumot manbasi" className="size-6 opacity-0 transition-opacity group-hover:opacity-100 focus-visible:opacity-100" size="sm" variant="ghost">
      ...
      <div className="flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5">
        <span className="inline-flex items-center gap-0.5 text-[11.5px] font-semibold" style={{
            color: isGood === null
                ? 'var(--viz-muted)'
                : isGood
                    ? 'var(--viz-delta-good)'
      ...
      <div className="mt-auto flex items-end gap-1.5">
        ...
        {caption && (<span className="shrink-0 text-[9px] leading-none text-muted">{caption}</span>)}
      </div>
    </article>);
}
/**
 ...
 */
export function MiniStat({ label, value, unit, hint, icon, tone = 'accent', delta, deltaGood, }) {
    const toneColor = tone === 'good' ? 'var(--viz-good)'
        : tone === 'warning' ? 'var(--viz-warning)'
            : tone === 'critical' ? 'var(--viz-critical)'
                : 'var(--accent)';
    const DeltaIcon = deltaGood === null || deltaGood === undefined ? Minus : deltaGood ? ArrowUp : ArrowDown;
    return (<div className="panel panel--row gap-3 px-3.5 py-3">
      {icon && (<span className="flex size-9 shrink-0 items-center justify-center rounded-lg" style={{
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-0.5 truncate text-[19px] font-bold leading-none" style={{ fontVariantNumeric: 'tabular-nums' }}>
          {value}
          {unit && <span className="ml-1 text-[10.5px] font-medium text-muted">{unit}</span>}
        </p>

        {(hint || delta) && (<p className="mt-1 flex items-center gap-2">
            <span className="min-w-0 flex-1 truncate text-[10.5px] text-muted">{hint ?? ''}</span>
            {delta && (<span className="inline-flex shrink-0 items-center gap-0.5 text-[10.5px] font-semibold" style={{
                    color: deltaGood === null || deltaGood === undefined
                        ? 'var(--viz-muted)'
                        : deltaGood
                            ? 'var(--viz-delta-good)'
                            : 'var(--viz-delta-bad)',
 ...
 */
export function QuickMetric({ label, value, icon, tone = 'accent', }) {
    const toneColor = tone === 'good' ? 'var(--viz-good)'
        : tone === 'warning' ? 'var(--viz-warning)'
            : tone === 'critical' ? 'var(--viz-critical)'
                : 'var(--accent)';
    return (<div className="flex items-start gap-2.5">
      {icon && (<span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-lg" style={{
                background: `color-mix(in oklab, ${toneColor} 12%, transparent)`,
                color: toneColor,
            }}>
          {icon}
...
export function StatusChip({ status, label, size = 'sm', }) {
    ...
    return (<Chip color={color} size={size} variant="soft">
