 ...
 */
export const CHART_FONT = '"Inter Variable", "Inter", "Segoe UI", system-ui, -apple-system, Arial, sans-serif';
function read(el, name) {
    return getComputedStyle(el).getPropertyValue(name).trim();
}
export function readVizTokens(el = document.documentElement) {
    const v = (n) => read(el, n);
    return {
...
const getSnapshot = () => cachedSnapshot || currentKey();
let tokenCache = null;
...
export function echartsTheme(t) {
    return {
 ...
 */
export function divergingColor(t, value, maxAbs) {
