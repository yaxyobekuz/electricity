...
export default function TpListPage() {
    ...
    const rows = useMemo(() => {
        ...
        if (mfyFilter)
            out = out.filter((r) => String(r.mfyId) === mfyFilter);
        if (conditionFilter)
            out = out.filter((r) => r.condition === conditionFilter);
        return out;
    ...
    return (<>
      <PageHeader actions={<PeriodPicker />} subtitle={`${tp.data?.length ?? 0} ta transformator · ${overloaded} ta ortiqcha yuklangan · ${nonCompliant} ta masofa normasidan uzoq`} title={t('nav.transformers')}/>

      {/* Filtr qatori — u ta'sir qiladigan jadvaldan YUQORIDA */}
      <div className="mb-3 flex flex-wrap items-end gap-2">
        <SearchField aria-label={t('common.search')} className="w-60" value={search} onChange={setSearch}>
          <SearchField.Group>
        ...
        <Select aria-label="Mahalla" className="w-52" placeholder="Barcha mahallalar" value={mfyFilter} onChange={(v) => setMfyFilter(v === null ? null : String(v))}>
          <Select.Trigger>
            <Select.Value />
            <Select.Indicator />
          </Select.Trigger>
        ...
        <Select aria-label="Holat" className="w-44" placeholder="Barcha holatlar" value={conditionFilter} onChange={(v) => setConditionFilter(v === null ? null : String(v))}>
          <Select.Trigger>
            <Select.Value />
            <Select.Indicator />
          </Select.Trigger>
