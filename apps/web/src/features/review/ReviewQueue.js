...
import { AlertDialog, Alert, Button, Chip, Label, Modal, TextArea, TextField, toast, } from '@heroui/react';
...
export default function ReviewQueue() {
    ...
    const approve = useMutation({
        mutationFn: (id) => api.post(`/entry/submission/${id}/approve`, {}),
        onSuccess: () => {
    ...
    const reject = useMutation({
        mutationFn: ({ id, note }) => api.post(`/entry/submission/${id}/reject`, { note }),
        onSuccess: () => {
    ...
    return (<>
      <PageHeader subtitle={`${rows.length} ta hisobot ko‘rib chiqishni kutmoqda`} title={t('entry.reviewQueue')}/>

      <Panel flush>
        {rows.length === 0 ? (<EmptyPanel message={t('entry.noSubmissions')}/>) : (<table className="dt">
            <thead>
              <tr>
                <th>Mahalla</th>
                <th>Ma’lumot turi</th>
              ...
              {rows.map((s) => (<tr key={s.id}>
                  ...
                  <td>
                    <div className="flex items-center gap-1">
                      ...
                      <Button isPending={approve.isPending && approve.variables === s.id} size="sm" onPress={() => approve.mutate(s.id)}>
      ...
      {/* Farq oynasi */}
      {selected && (<Modal.Backdrop isOpen onOpenChange={(open) => !open && setSelected(null)}>
          <Modal.Container size="lg">
            <Modal.Dialog>
              ...
              <Modal.Body>
                ...
                <div className="mt-4">
                  <TextField className="w-full" value={rejectNote} onChange={setRejectNote}>
                    <Label className="text-xs">Rad etish sababi (rad etish uchun majburiy)</Label>
                    <TextArea placeholder="Masalan: 12–14-kunlarda tarmoqqa kirgan energiya qiymati shubhali…" rows={2}/>
                  </TextField>
                </div>
              </Modal.Body>
              <Modal.Footer>
                <Button isDisabled={rejectNote.trim().length < 5} variant="danger" onPress={() => reject.mutate({ id: selected.id, note: rejectNote })}>
                  <X className="size-4"/>
                  Rad etish
                </Button>
...
function DiffTable({ submissionId }) {
    const diff = useQuery({
        queryKey: ['entry', 'diff', submissionId],
        queryFn: () => api.get(`/entry/submission/${submissionId}/diff`),
    });
    if (diff.isLoading)
        return <LoadingState rows={2}/>;
    const rows = diff.data ?? [];
    if (rows.length === 0) {
        return (<p className="rounded-lg border border-border/70 bg-surface-secondary px-3 py-2.5 text-xs text-muted">
        Bu birinchi revisiya — taqqoslash uchun oldingi tasdiqlangan hisobot yo‘q.
      </p>);
    }
    return (<div className="rounded-lg border border-border/70">
      <table className="dt">
        <thead>
          <tr>
            <th>Ko‘rsatkich</th>
        ...
        <tbody>
          {rows.map((r) => {
            ...
            return (<tr key={r.path} className={big ? 'bg-warning/10' : undefined}>
                <td>{r.labelUz}</td>
                ...
                <td className="num">
                  {r.deltaPct === null ? ('—') : (<span className="font-medium" style={{
