...
import { Building2, CalendarDays, FileSpreadsheet, FileText, Printer, ScrollText, } from 'lucide-react';
...
export default function ReportsPage() {
    ...
    return (<div>
      <PageHeader actions={<PeriodPicker />} subtitle="davriy hisobotlar va rasmiy pasportlar" title={t('nav.reports')}/>

      <div className="mb-2.5 grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        {/* ── Davriy hisobot ─────────────────────────────────────────── */}
        <Panel className="xl:col-span-5" subtitle="tanlangan davr bo‘yicha to‘liq ma’lumot" title="Davriy hisobot">
          <div className="flex flex-col gap-3">
            <div>
              ...
              <ToggleButtonGroup aria-label="Hisobot davri" className="flex-wrap" selectedKeys={new Set([kind])} selectionMode="single" size="sm" onSelectionChange={(keys) => {
            const next = [...keys][0];
            if (typeof next === 'string')
                setKind(next);
        }}>
                {PERIOD_KINDS.map((p) => (<ToggleButton key={p.id} id={p.id}>
                    {t(p.labelKey)}
                  </ToggleButton>))}
              </ToggleButtonGroup>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <Button size="sm" variant="secondary" onPress={() => go(`/api/report/period/${kind}.xlsx${qs}`)}>
                <FileSpreadsheet className="size-4 text-viz-3"/>
                {t('reports.excel')}
              </Button>
              <Button size="sm" variant="secondary" onPress={() => go(`/api/report/period/${kind}.pdf${qs}`)}>
        ...
        {/* ── Tuman pasporti ─────────────────────────────────────────── */}
        <Panel className="xl:col-span-7" subtitle="rasmiy hujjat — 13 qatorli forma" title="Tuman pasporti">
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <Button size="sm" variant="secondary" onPress={() => go(`/api/report/passport/tuman.xlsx${qs}`)}>
                <FileSpreadsheet className="size-4 text-viz-3"/>
                Excel
              </Button>
              <Button size="sm" variant="secondary" onPress={() => go(`/api/report/passport/tuman.pdf${qs}`)}>
                <FileText className="size-4 text-viz-2"/>
                PDF
              </Button>
              <Button size="sm" variant="ghost" onPress={() => window.open(`/passport/print/tuman/0/${period ?? 'latest'}`, '_blank')}>
      ...
      {/* ── Mahalla pasportlari ──────────────────────────────────────── */}
      <Panel actions={<SearchField aria-label={t('common.search')} className="w-52" value={search} onChange={setSearch}>
            <SearchField.Group>
              <SearchField.SearchIcon />
              <SearchField.Input placeholder="Mahalla nomi…"/>
              <SearchField.ClearButton />
            </SearchField.Group>
          </SearchField>} subtitle={`${mfys.length} ta mahalla`} title="Mahalla pasportlari">
        {mfys.length === 0 ? (<EmptyPanel message="Mahalla topilmadi"/>) : (<ul className="grid grid-cols-1 gap-2 sm:grid-cols-2 xl:grid-cols-3">
            {mfys.map((m) => (<li key={m.id} className="flex items-center gap-2.5 rounded-lg bg-surface-secondary px-3 py-2">
                ...
                <span className="flex shrink-0 items-center gap-0.5">
                  <Button isIconOnly aria-label={`${m.nameUz} — pasportni ochish`} size="sm" variant="ghost" onPress={() => void navigate(`/passport/mfy/${m.id}/${period ?? 'latest'}`)}>
                    <ScrollText className="size-3.5"/>
                  </Button>
                  <Button isIconOnly aria-label={`${m.nameUz} — Excel`} size="sm" variant="ghost" onPress={() => go(`/api/report/passport/mfy/${m.id}.xlsx${qs}`)}>
                    <FileSpreadsheet className="size-3.5 text-viz-3"/>
                  </Button>
                  <Button isIconOnly aria-label={`${m.nameUz} — PDF`} size="sm" variant="ghost" onPress={() => go(`/api/report/passport/mfy/${m.id}.pdf${qs}`)}>
