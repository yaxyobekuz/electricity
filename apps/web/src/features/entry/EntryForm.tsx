 ...
 *
 * ENERGY_BALANCE   → 30 ta forma emas, BITTA OYLIK JADVAL (qator = kun)
 * MONTHLY_RETURN   → pasportning 1, 5, 6, 7, 13-qatorlari
 *
 * Umumiy qoidalar:
...
import { TotalsBar, type BalanceTotals } from './TotalsBar.tsx';
...
interface SubmissionResponse {
  ...
  data: EnergyBalanceDay[] | MonthlyReturn | null;
}
...
export default function EntryForm() {
  ...
  const draft = useQuery({
    ...
    queryFn: () =>
      api.post<Submission>('/entry/submission', {
  ...
  const detail = useQuery({
    ...
    queryFn: () => api.get<SubmissionResponse>(`/entry/submission/${submissionId}`),
  ...
  if (draft.isLoading || detail.isLoading) return <LoadingState rows={5} />;
  if (draft.isError) {
    return (
      <ErrorState
        message={
          draft.error instanceof ApiRequestError
            ? draft.error.message
            : 'Qoralama ochib bo‘lmadi'
        }
  ...
  if (!submission) return <ErrorState message="Konvert topilmadi" />;
  ...
  return (
    <>
      ...
      {domain === 'ENERGY_BALANCE' ? (
        <EnergyBalanceGrid
          initial={(detail.data?.data as EnergyBalanceDay[]) ?? []}
          period={period}
          readOnly={readOnly}
          submission={submission}
          validation={detail.data?.validation}
          onSaved={() => void qc.invalidateQueries({ queryKey: ['entry'] })}
        />
      ) : domain === 'MONTHLY_RETURN' ? (
        <MonthlyReturnForm
          initial={(detail.data?.data as MonthlyReturn) ?? null}
          readOnly={readOnly}
          submission={submission}
          validation={detail.data?.validation}
          onSaved={() => void qc.invalidateQueries({ queryKey: ['entry'] })}
        ...
        <Panel>
          <p className="text-sm text-muted">
            «{DOMAIN_LABEL_UZ[domain as keyof typeof DOMAIN_LABEL_UZ] ?? domain}» formasi
...
function EnergyBalanceGrid({
  submission, period, initial, readOnly, validation, onSaved,
}: {
  submission: Submission;
  period: string;
  initial: EnergyBalanceDay[];
  readOnly: boolean;
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: EnergyBalanceDay[]) =>
      api.patch<{ saved: number; validation: ValidationReport; savedAt: string }>(
        `/entry/submission/${submission.id}/energy-balance`,
        { rows: payload },
      ),
  ...
  const scheduleSave = useCallback(
    (next: Record<string, EnergyBalanceDay>) => {
      ...
      debounceRef.current = setTimeout(() => {
        ...
        if (filled.length > 0) saveMutation.mutate(filled);
      }, 800);
    },
  ...
  const update = (date: string, patch: Partial<EnergyBalanceDay>): void => {
    setRows((prev) => {
  ...
  return (
    <>
      <Panel
        ...
        subtitle="Qator = kun. «Jami yo‘qotish» ustuni hisoblanadi va tahrirlanmaydi."
        title="Kunlik energiya balansi"
        flush
      >
        {/* Tekshiruv xabarlari */}
  ...
  return (
    <td className="num p-1">
      <NumberField
        ...
        minValue={0}
        value={value}
        onChange={(v) => onChange(Number.isFinite(v) ? (v ?? 0) : 0)}
      >
        <NumberField.Group>
...
function MonthlyReturnForm({
  submission, initial, readOnly, validation, onSaved,
}: {
  submission: Submission;
  initial: MonthlyReturn | null;
  readOnly: boolean;
  validation: ValidationReport | undefined;
  onSaved: () => void;
}) {
  const navigate = useNavigate();
  const [data, setData] = useState<MonthlyReturn>(initial ?? EMPTY_RETURN);
  const [fieldErrors, setFieldErrors] = useState<Record<string, string>>({});
  const [saveState, setSaveState] = useState<'idle' | 'saving' | 'saved' | 'error'>('idle');
  const [savedAt, setSavedAt] = useState<string | null>(null);

  const save = useMutation({
    mutationFn: (payload: MonthlyReturn) =>
      api.patch<{ savedAt: string }>(`/entry/submission/${submission.id}/monthly-return`, payload),
    onMutate: () => setSaveState('saving'),
    onSuccess: (res) => {
      setSaveState('saved');
      setSavedAt(res.savedAt);
  ...
  const set = (patch: Partial<MonthlyReturn>): void => setData((d) => ({ ...d, ...patch }));
    ...
    <div className="flex flex-col gap-3">
      ...
      <Panel subtitle="Pasport 1-qatori" title="Iste’molchilar">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <NumInput
            error={fieldErrors['consumersPopulation']}
            label="Aholi"
            readOnly={readOnly}
            unit="ta"
            value={data.consumersPopulation}
            onChange={(v) => set({ consumersPopulation: v })}
          />
          <NumInput
            error={fieldErrors['consumersLegal']}
            label="Yuridik"
            readOnly={readOnly}
            unit="ta"
            value={data.consumersLegal}
            onChange={(v) => set({ consumersLegal: v })}
          />
          ...
          <NumInput
            ...
            onChange={(v) => set({ consumersActive: v })}
          />
          <NumInput
            error={fieldErrors['consumersDisconnected']}
            label="Uzilgan"
            readOnly={readOnly}
            unit="ta"
            value={data.consumersDisconnected}
            onChange={(v) => set({ consumersDisconnected: v })}
          />
        </div>
      </Panel>
      ...
      <Panel subtitle="Pasport 5-qatori" title="Debitor qarzdorlik">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <NumInput
            decimals={1}
            error={fieldErrors['debtPopulationMln']}
            label="Aholi"
            readOnly={readOnly}
            unit="mln so‘m"
            value={data.debtPopulationMln}
            onChange={(v) => set({ debtPopulationMln: v })}
          />
          <NumInput
            decimals={1}
            error={fieldErrors['debtLegalMln']}
            label="Yuridik"
            readOnly={readOnly}
            unit="mln so‘m"
            value={data.debtLegalMln}
            onChange={(v) => set({ debtLegalMln: v })}
          />
          <NumInput
            decimals={1}
            error={fieldErrors['debtBudgetMln']}
            label="Budjet tashkilotlari"
            readOnly={readOnly}
            unit="mln so‘m"
            value={data.debtBudgetMln}
            onChange={(v) => set({ debtBudgetMln: v })}
          />
      ...
      <Panel subtitle="Pasport 6, 7, 13-qatorlari" title="Hisoblagichlar">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <NumInput
            error={fieldErrors['metersOfflineCnt']}
            label="Aloqadan chiqqan"
            readOnly={readOnly}
            unit="ta"
            value={data.metersOfflineCnt}
            onChange={(v) => set({ metersOfflineCnt: v })}
          />
          <NumInput
            error={fieldErrors['lowConsumptionCnt']}
            hint="0 va 50 kWh dan kam"
            label="Kam iste’molchilar"
            readOnly={readOnly}
            unit="ta"
            value={data.lowConsumptionCnt}
            onChange={(v) => set({ lowConsumptionCnt: v })}
          />
          <NumInput
            error={fieldErrors['metersReplaceNeedCnt']}
            label="Almashtirish kerak"
            readOnly={readOnly}
            unit="ta"
            value={data.metersReplaceNeedCnt}
            onChange={(v) => set({ metersReplaceNeedCnt: v })}
          />
          <NumInput
            error={fieldErrors['metersReplacedCnt']}
            label="Almashtirilgan"
            readOnly={readOnly}
            unit="ta"
            value={data.metersReplacedCnt}
            onChange={(v) => set({ metersReplacedCnt: v })}
          />
        </div>
      </Panel>

      {validation && validation.issues.length > 0 && (
        <Alert status={validation.ok ? 'warning' : 'danger'}>
          <Alert.Indicator />
          <Alert.Content>
      ...
      {!readOnly && (
        <div className="sticky bottom-0 -mx-4 flex items-center justify-between gap-3 border-t border-border bg-surface/97 px-4 py-2.5 backdrop-blur">
...
function NumInput({
  ...
  return (
    <NumberField
      className="w-full"
      formatOptions={{ style: 'decimal', maximumFractionDigits: decimals }}
      isDisabled={readOnly}
    ...
    >
      ...
      {error ? (
        ...
        <Description className="text-[10px]">{hint}</Description>
      ) : null}
    </NumberField>
  );
}
...
function ComputedField({
  ...
  return (
    <TextField isDisabled className="w-full" value={num(value, decimals)}>
