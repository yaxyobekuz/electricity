...
export default function EntryPage() {
    ...
    const effectivePeriod = period ?? (boot.data?.dataRange.maxDate ? toPeriod(boot.data.dataRange.maxDate) : null);
    ...
    if (completeness.isLoading || boot.isLoading)
        return <LoadingState rows={5}/>;
    const total = mfys.length * DOMAINS.length;
    const approvedPct = total > 0 ? Math.round(((stats['approved'] ?? 0) / total) * 100) : 0;
    return (<>
      <PageHeader actions={<PeriodPicker />} subtitle={effectivePeriod
            ? `${periodLabel(effectivePeriod)} · ${approvedPct}% tasdiqlangan`
            : undefined} title={t('entry.title')}/>

      {/* Umumiy holat */}
      <div className="mb-4 flex flex-wrap gap-2">
        {['approved', 'submitted', 'draft', 'rejected', 'missing'].map((s) => {
            const meta = STATUS_META[s];
            const Icon = meta.icon;
            return (<div key={s} className="flex items-center gap-2 rounded-lg border border-border/70 bg-surface px-3 py-2">
      ...
      <Panel subtitle="Mahalla × ma’lumot turi. Katakcha ustiga bosib formani oching." title={t('entry.completeness')} flush>
          ...
          <table className="dt min-w-[900px]">
            <thead>
              <tr>
                ...
                {DOMAINS.map((d) => (<th key={d} className="text-center">
            ...
            <tbody>
              {mfys.map((m) => (<tr key={m.id}>
                ...
                return (<td key={d} className="p-1 text-center">
                        <Tooltip delay={300}>
                          <button type="button" aria-label={`${m.nameUz} — ${DOMAIN_LABEL_UZ[d]}: ${meta.label}`} className={cn('inline-flex size-8 items-center justify-center rounded-lg transition-transform', canOpen ? 'cursor-pointer hover:scale-110' : 'cursor-default')} style={{ background: meta.bg }} disabled={!canOpen} onClick={() => {
                        if (canOpen && effectivePeriod) {
                          ...
                          <Tooltip.Content>
                            <p className="text-xs font-medium">{DOMAIN_LABEL_UZ[d]}</p>
      ...
      {!canWrite && (<Chip className="mt-3" size="sm" variant="soft">
          <Chip.Label>
