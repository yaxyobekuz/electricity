...
export default function MfyList() {
    ...
    const [sort, setSort] = useState({ column: 'lossPct', direction: 'descending' });
    const rows = useMemo(() => {
    ...
    if (lossMap.isLoading)
        return <LoadingState rows={5}/>;
    return (<>
      <PageHeader actions={<div className="flex items-center gap-2">
            <SearchField aria-label={t('common.search')} className="w-56" value={search} onChange={setSearch}>
              <SearchField.Group>
            ...
            <PeriodPicker />
          </div>} subtitle={`${rows.length} ta mahalla`} title={t('nav.mahallalar')}/>

      <Panel flush>
        <Table>
          <Table.ScrollContainer>
            <Table.Content aria-label="Mahallalar" className="min-w-[760px]" sortDescriptor={sort} onSortChange={setSort}>
              <Table.Header>
              ...
              <Table.Body>
                {rows.map((r) => (<Table.Row key={r.mfyId} id={r.mfyId} onAction={() => void navigate(`/dashboard/mfy/${r.mfyId}`)}>
                    <Table.Cell>
                    ...
                    <Table.Cell>
                      <span className="tabular font-medium" style={{
                color: r.gapPp > 0 ? 'var(--viz-critical)' : 'var(--viz-good)',
                    ...
                    <Table.Cell>
                      <Chip color={r.status === 'good' ? 'success'
