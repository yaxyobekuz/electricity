...
import { Activity, Building2, CircleDollarSign, Gauge as GaugeIcon, Leaf, Power, TrendingDown, Users, Zap, ZapOff, } from 'lucide-react';
...
import { MiniStat, QuickMetric, StatTile, } from '../../components/ui/StatTile.tsx';
...
import { useAlerts, useMfyCapacity, useMfyConsumers, useMfyDebt, useMfyDynamics, useMfyLossStructure, useMfyOperational, useMfyOverview, useMfyResults, useMfyTp, useMfyWorks, } from '../../lib/queries.ts';
...
export default function MfyDashboard() {
    ...
    const dynamics = useMfyDynamics(mfyId, { bucket });
    ...
    if (overview.isError || !overview.data) {
        return (<ErrorState message={overview.error instanceof Error ? overview.error.message : 'Ma’lumot topilmadi'} onRetry={() => void overview.refetch()}/>);
    }
    ...
    const avgPerConsumer = totals.consumersActive > 0 ? totals.kwhSold / totals.consumersActive / daysInMonth : 0;
    ...
    return (<div className={overview.isFetching ? 'opacity-70 transition-opacity' : ''}>
      <PageHeader actions={<>
            <PeriodPicker />
      ...
      <div className="mb-2.5 grid grid-cols-2 gap-2.5 md:grid-cols-4 xl:grid-cols-8">
        {overview.data.tiles.map((tile) => (<StatTile key={tile.key} icon={TILE_ICONS[tile.key]} tile={tile} tone={TILE_TONES[tile.key] ?? 'blue'}/>))}
      </div>

      {/* ═══ 2-QATOR: dinamika · ogohlantirish · quvvat ══════════════════ */}
      <div className="mb-2.5 grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        <Panel actions={<BucketPicker onChange={setBucket} value={bucket}/>} className="xl:col-span-5" title="Iste’mol va yo‘qotish dinamikasi">
          {series.length > 0 ? (<div className="flex flex-col gap-3 lg:flex-row">
              <div className="min-w-0 flex-1">
                <TrendLine bucket={bucket} csvName={`${mfy.code}-dinamika`} height={214} points={series}/>
              </div>

              {/* Maketdagi yon xulosa ustuni */}
              <div className="grid shrink-0 grid-cols-2 gap-2 lg:w-42 lg:grid-cols-1">
                <SummaryBox label={`${periodWord} tarmoqqa kirgan`} tone="var(--viz-1)" value={num(today?.kwhIn ?? 0, 0)}/>
                <SummaryBox label={`${periodWord} sotilgan`} tone="var(--viz-3)" value={num(today?.kwhSold ?? 0, 0)}/>
                <SummaryBox extra={`${(today?.lossPct ?? 0).toFixed(1)}%`} label={`${periodWord} yo‘qotish`} tone="var(--viz-5)" value={num(today?.kwhLoss ?? 0, 0)}/>
                <SummaryBox extra={`${(yesterday?.lossPct ?? 0).toFixed(1)}%`} label={`${prevWord} yo‘qotish`} muted value={num(yesterday?.kwhLoss ?? 0, 0)}/>
              </div>
            </div>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel className="xl:col-span-4" flush subtitle="qoidalarga asoslangan tahlil" title="Diqqat talab qiladigan holatlar">
          <AlertsPanel items={mfyAlerts}/>
        </Panel>

        <Panel className="xl:col-span-3" title="Tarmoq quvvati">
          {capacity.data ? (<div className="flex flex-col gap-2">
              <Gauge height={124} higherIsBetter={false} label="Joriy yuklama" suffix="%" value={capacity.data.loadPct}/>
              ...
              <dl className="grid grid-cols-3 gap-1 text-center">
                ...
                <CapacityCell label="Zaxira" tone="var(--viz-good)" value={kva(capacity.data.reserveKva)}/>
              </dl>

              {operational.data && (<div className="grid grid-cols-2 gap-x-2 gap-y-2.5 border-t border-separator pt-2.5">
                  <QuickMetric icon={<Zap className="size-3.5"/>} label="Maks. yuklama" value={kw(operational.data.maxLoadKw)}/>
                  <QuickMetric icon={<Activity className="size-3.5"/>} label="Min. yuklama" tone="good" value={kw(operational.data.minLoadKw)}/>
                  <QuickMetric icon={<GaugeIcon className="size-3.5"/>} label="O‘rt. kuchlanish" tone={operational.data.avgVoltageV !== null &&
                    Math.abs(operational.data.avgVoltageV - operational.data.nominalVoltageV) >
                        operational.data.nominalVoltageV * 0.1
                    ? 'warning'
                    : 'accent'} value={volts(operational.data.avgVoltageV)}/>
                  <QuickMetric icon={<Power className="size-3.5"/>} label="O‘chirishlar" tone={(operational.data.outageCount ?? 0) > 10 ? 'warning' : 'good'} value={`${num(operational.data.outageCount ?? 0)} ta`}/>
                </div>)}
            </div>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>
      </div>
      ...
      <div className="mb-2.5 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <MiniStat delta={`${num(consumers.data?.new ?? 0)} ta`} deltaGood hint={`Jami: ${num(totals.consumersTotal)} ta`} icon={<Building2 className="size-4.5"/>} label="Yuridik iste’molchilar" unit="ta" value={num(consumers.data?.legal ?? 0)}/>
        <MiniStat hint={`Faol abonent: ${num(totals.consumersActive)} ta`} icon={<Activity className="size-4.5"/>} label="O‘rtacha iste’mol (1 abonent)" unit="kWh/kun" value={num(avgPerConsumer, 1)}/>
        <MiniStat hint="taxminiy tarif bo‘yicha" icon={<CircleDollarSign className="size-4.5"/>} label="O‘rtacha hisob (1 abonent)" unit="so‘m" value={num(avgBillSum, 0)}/>
        <MiniStat hint={`Yo‘qotishdagi ulushi: ${pct(lossStructure.data?.parts[0]?.pct ?? 0, 1)}`} icon={<Leaf className="size-4.5"/>} label="Tabiiy yo‘qotish" tone="good" unit={energy(naturalKwh).unit} value={num(energy(naturalKwh).value, 1)}/>
      </div>
      ...
      <div className="mb-2.5 grid grid-cols-1 gap-2.5 md:grid-cols-2 xl:grid-cols-12">
        <Panel className="xl:col-span-3" title="Abonentlar holati">
          {consumers.data ? (<Donut centerLabel="jami abonent" centerValue={num(consumers.data.total)} csvName={`${mfy.code}-abonentlar`} height={166} slices={[
        ...
        <Panel actions={<Chip size="sm" variant="soft">
              <Chip.Label>{num(tp.data?.length ?? 0)} ta</Chip.Label>
            </Chip>} className="md:col-span-2 xl:col-span-6" flush footerAction={{ label: 'Barcha transformatorlar', to: '/transformers' }} title="Transformatorlar holati">
          <TpMonitorPanel rows={(tp.data ?? []).slice(0, 6)} showMfy={false}/>
        </Panel>

        <Panel className="xl:col-span-3" title="Yo‘qotishlar tuzilmasi">
          {lossStructure.data ? (<Donut centerLabel="jami yo‘qotish" centerValue={energy(lossStructure.data.totalKwh).text} csvName={`${mfy.code}-yoqotish`} formatValue={(v) => energy(v).text} height={166} slices={lossStructure.data.parts.map((p) => ({
                id: p.key,
                label: p.labelUz,
                value: p.kwh,
      ...
      <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2 xl:grid-cols-12">
        <Panel className="xl:col-span-3" footerAction={{ label: 'Qarzdorlar ro‘yxati', to: '/debt' }} title="Qarzdorlik tuzilmasi">
          {debt.data ? (<Donut centerLabel="jami qarzdorlik" centerValue={money(debt.data.totalMln).text} csvName={`${mfy.code}-qarzdorlik`} formatValue={(v) => money(v).text} height={166} slices={debt.data.byCategory.map((c) => ({
                id: c.category,
                label: c.labelUz,
                value: c.amountMln,
        ...
        <Panel actions={<Chip size="sm" variant="soft">
            ...
            </Chip>} className="md:col-span-2 xl:col-span-6" flush footerAction={{ label: 'Barcha ishlar', to: '/works' }} title="Ishlar">
        ...
        <Panel className="xl:col-span-3" title="Natijadorlik">
          {results.data ? (<div className="flex flex-col gap-2.5">
              <div className="flex items-center gap-2">
                <span className="text-[19px] font-bold leading-none" style={{ fontVariantNumeric: 'tabular-nums' }}>
                  {pct(results.data.lossPctStart ?? 0, 1)}
                </span>
                <TrendingDown aria-hidden="true" className="size-4 text-muted"/>
                <span className="text-[19px] font-bold leading-none" style={{ fontVariantNumeric: 'tabular-nums', color: 'var(--viz-good)' }}>
              ...
              {results.data.improvementPp !== null && (<p className="text-[11.5px] font-semibold" style={{
                    color: results.data.improvementPp > 0
                        ? 'var(--viz-delta-good)'
                        : 'var(--viz-delta-bad)',
                }}>
              ...
              <div className="flex items-start gap-2 border-t border-separator pt-2.5">
                ...
                <span className="min-w-0">
                  <span className="block text-[15px] font-bold leading-tight" style={{ fontVariantNumeric: 'tabular-nums' }}>
      ...
      <p className="mt-2.5 flex items-center gap-1.5 text-[10.5px] text-muted">
        ...
        <button className="font-semibold text-accent hover:underline" type="button" onClick={() => void navigate(`/passport/mfy/${mfyId}/${period ?? 'latest'}`)}>
          Pasportni ochish
        </button>
      </p>
    </div>);
 ...
 */
function SummaryBox({ label, value, extra, tone, muted, }) {
    return (<div className={muted ? 'rounded-lg bg-surface-secondary px-3 py-2' : 'px-1 py-1'}>
      <p className="truncate text-[10.5px] leading-tight text-muted">{label}</p>
      <p className="mt-1 flex items-baseline gap-1">
        <span className="truncate text-[15px] font-bold leading-none" style={{ fontVariantNumeric: 'tabular-nums', color: muted ? undefined : tone }}>
          {value}
        </span>
        <span className="shrink-0 text-[10px] font-medium text-muted">kWh</span>
        {extra && (<span className="shrink-0 text-[10.5px] font-semibold" style={{ color: muted ? 'var(--viz-muted)' : tone }}>
            ({extra})
          </span>)}
      </p>
    </div>);
...
export function BucketPicker({ value, onChange, }) {
    return (<Select aria-label="Diagramma davri" className="w-28" selectedKey={value} onSelectionChange={(k) => onChange(k)}>
...
function CapacityCell({ label, value, tone }) {
    return (<div>
      <dt className="truncate text-[9.5px] leading-tight text-muted">{label}</dt>
      <dd className="truncate text-[12px] font-bold leading-tight" style={{ fontVariantNumeric: 'tabular-nums', color: tone }}>
        {value}
      </dd>
    </div>);
}
