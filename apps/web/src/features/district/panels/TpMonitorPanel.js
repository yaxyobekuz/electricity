...
export function TpMonitorPanel({ rows, showMfy = true }) {
    const t = useVizTokens();
    if (rows.length === 0)
        return <EmptyPanel message="Ma’lumot yo‘q"/>;
    return (<div className="scroll-y max-h-75 overflow-x-auto">
      {/*
      ...
      <table className="dt min-w-105 table-fixed">
        ...
        <tbody>
          {rows.map((r) => {
            ...
            return (<tr key={r.tpId}>
                <td className="truncate">
                  <Link to={`/dashboard/mfy/${r.mfyId}`} className="block truncate text-[12.5px] font-medium hover:underline">
                    {r.code}
                  </Link>
                  {showMfy && (<span className="block truncate text-[10px] leading-tight text-muted">
                ...
                <td>
                  <div className="flex items-center gap-1.5">
                    <span className="loadbar flex-1">
                      <span className="loadbar__fill" style={{ width: `${Math.min(100, r.loadPct)}%`, background: color }}/>
                      {/* Optimal chegara belgisi */}
                      <span className="loadbar__mark" style={{ left: `${Math.min(100, r.optimalPct)}%` }} aria-hidden="true"/>
                    </span>
                ...
                <td className="num whitespace-nowrap text-[12.5px]">
                  <span className="inline-flex items-center gap-1">
                    ...
                    {r.distanceCompliant === true && (<Check className="size-3 shrink-0 text-viz-good" aria-label="Normaga mos"/>)}
                    {r.distanceCompliant === false && (<X className="size-3 shrink-0 text-viz-critical" aria-label="Normadan uzoq"/>)}
                  </span>
                </td>
                <td>
                  <Chip className="whitespace-nowrap" color={status === 'good' ? 'success' : status === 'warning' ? 'warning' : 'danger'} size="sm" variant="soft">
