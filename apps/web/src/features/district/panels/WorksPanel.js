...
export function WorksPanel({ rows, mode }) {
    const t = useVizTokens();
    if (rows.length === 0) {
        return (<EmptyPanel message={mode === 'planned' ? 'Rejalashtirilgan ish yo‘q' : 'Bajarilgan ish yo‘q'}/>);
    }
    // Bajarilgan ishlarning umumiy natijasi
    const totalSaved = rows.reduce((a, r) => a + r.effectSavingKwhMonth, 0);
    const withEffect = rows.find((r) => r.effectLossPctBefore !== null && r.effectLossPctAfter !== null);
    return (<div className="flex flex-col">
      <ul className="scroll-y max-h-[248px] divide-y divide-separator/50">
        {rows.slice(0, 12).map((r) => (<li key={r.id} className="px-4 py-2.5">
            <div className="flex items-start justify-between gap-2">
              ...
              <Chip className="shrink-0" color={r.status === 'COMPLETED' ? 'success'
                : r.status === 'IN_PROGRESS' ? 'accent' : 'default'} size="sm" variant="soft">
                <Chip.Label>{STATUS_LABEL[r.status] ?? r.status}</Chip.Label>
              </Chip>
            </div>

            {mode === 'planned' && (<div className="mt-1.5 flex items-center gap-2">
            ...
            {mode === 'completed' && r.effectSavingKwhMonth > 0 && (<p className="mt-1 text-[11px] text-viz-good">
      ...
      {mode === 'completed' && (totalSaved > 0 || withEffect) && (<div className="border-t border-separator/50 px-4 py-3">
