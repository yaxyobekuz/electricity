/**
 * Masofa analitikasi — TP dan iste'molchigacha.
 *
 * "Bullet" ko'rinishi: o'lchov = o'rtacha masofa, marker = 300 m normasi.
 * Uzoq masofa texnik yo'qotishning bevosita sababi.
 */
import type { DistanceRow } from '@beap/shared';
          ...
          {rows.map((r) => {
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    <span className="loadbar h-2 flex-1">
                      ...
                      <span
                        ...
                        style={{ left: `${normPct}%`, background: t.ink2, width: '2px' }}
                        aria-hidden="true"
                      />
                    </span>
                ...
                <td className="num">
                  {r.tpOverStandard > 0 ? (
                    <span className="font-medium" style={{ color: t.status.critical }}>
                      {r.tpOverStandard} / {r.tpCount} ta
                    </span>
                  ) : (
                    <span className="text-muted">0 / {r.tpCount} ta</span>
                  )}
                </td>
                <td>
                  {r.compliant ? (
                    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-viz-good">
                      <Check className="size-3.5" /> Mos
                    </span>
                  ) : (
                    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-viz-critical">
