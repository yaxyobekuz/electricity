...
export function PeriodPicker() {
    ...
    const periods = useMemo(() => {
        if (!boot?.dataRange.minDate || !boot.dataRange.maxDate)
            return [];
    ...
    return (<div className="flex items-center gap-1">
      <Button isIconOnly aria-label="Oldingi oy" isDisabled={idx < 0 || idx >= periods.length - 1} size="sm" variant="ghost" onPress={() => setPeriod(periods[idx + 1] ?? null)}>
        <ChevronLeft className="size-4"/>
      </Button>

      <Select aria-label="Davr" className="w-40" placeholder="Davr tanlang" value={current} onChange={(v) => setPeriod(v === null ? null : String(v))}>
        <Select.Trigger>
        ...
        <Select.Popover>
          <ListBox>
            {periods.map((p) => (<ListBox.Item key={p} id={p} textValue={periodLabel(p)}>
                {periodLabel(p)}
      ...
      <Button isIconOnly aria-label="Keyingi oy" isDisabled={idx <= 0} size="sm" variant="ghost" onPress={() => setPeriod(periods[idx - 1] ?? null)}>
