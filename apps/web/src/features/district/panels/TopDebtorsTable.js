...
export function TopDebtorsTable({ rows }) {
    if (rows.length === 0)
        return <EmptyPanel message="Qarzdorlar ro‘yxati bo‘sh"/>;
    const max = Math.max(...rows.map((r) => r.amountMln), 1);
    return (<div className="scroll-y max-h-[280px]">
      <table className="dt">
        <thead>
          <tr>
            <th className="w-8">№</th>
        ...
        <tbody>
          {rows.map((r) => (<tr key={`${r.rank}-${r.debtorName}`}>
