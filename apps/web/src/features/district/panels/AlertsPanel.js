...
export function AlertsPanel({ items }) {
    ...
    if (items.length === 0) {
        return (<div className="flex flex-col items-center gap-2 px-4 py-10 text-center">
    ...
    return (<div className="flex flex-col">
      <ul className="scroll-y max-h-[276px] divide-y divide-separator/50">
        {items.map((item) => {
            ...
            const body = (<div className="flex gap-2.5 px-4 py-2.5">
              <Icon className="mt-0.5 size-4 shrink-0" style={{ color: meta.color }} aria-hidden="true"/>
              ...
              <Chip className="mt-0.5 shrink-0" color={item.severity === 'critical' || item.severity === 'serious' ? 'danger'
                    : item.severity === 'warning' ? 'warning' : 'default'} size="sm" variant="soft">
                <Chip.Label>{meta.label}</Chip.Label>
              </Chip>
            </div>);
            return (<li key={item.id}>
              {item.href ? (<Link to={item.href} className={cn('block transition-colors hover:bg-surface-secondary')}>
                  {body}
                </Link>) : (body)}
            </li>);
        })}
