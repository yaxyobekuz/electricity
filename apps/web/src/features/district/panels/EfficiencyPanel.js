...
function scoreLabel(score) {
    if (score >= 85)
        return { label: 'Yaxshi', tone: 'good' };
    if (score >= 70)
        return { label: 'Qoniqarli', tone: 'warning' };
    if (score >= 50)
        return { label: 'Past', tone: 'serious' };
    return { label: 'Tanqidiy', tone: 'critical' };
}
export function EfficiencyPanel({ data }) {
    const { t } = useTranslation();
    const tokens = useVizTokens();
    const { label, tone } = scoreLabel(data.score);
    const forecastFirst = data.forecast?.[0];
    const forecastLast = data.forecast?.at(-1);
    const improving = forecastFirst && forecastLast ? forecastLast.lossPct < forecastFirst.lossPct : null;
    return (<div className="flex flex-col gap-3">
      <Gauge height={182} label={t('efficiency.outOf')} value={data.score}/>

      <div className="flex items-center justify-center gap-2">
        <Chip color={tone === 'good' ? 'success' : tone === 'warning' ? 'warning' : 'danger'} size="sm" variant="soft">
          ...
          <Popover.Content className="max-w-80">
            <Popover.Dialog>
              ...
              <table className="dt text-[11px]">
                ...
                <tbody>
                  {data.components.map((c) => (<tr key={c.key}>
                      <td>{c.labelUz}</td>
      ...
      <ul className="flex flex-col gap-1.5">
        {data.components.map((c) => (<li key={c.key} className="flex items-center gap-2 text-[11px]">
            ...
            <span className="loadbar flex-1">
              <span className="loadbar__fill" style={{
                ...
                background: c.score >= 85 ? tokens.status.good
      ...
      {data.forecast && data.forecast.length > 0 && (<div className="rounded-lg border border-border/70 bg-surface-secondary p-2.5">
          <p className="mb-1 flex items-center gap-1.5 text-[11px] font-semibold">
            {improving ? (<TrendingDown className="size-3.5 text-viz-good"/>) : (<TrendingUp className="size-3.5 text-viz-critical"/>)}
