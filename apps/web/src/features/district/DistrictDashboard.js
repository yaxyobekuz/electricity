...
import { BatteryWarning, CircleDollarSign, Gauge as GaugeIcon, PlugZap, Ruler, ShoppingCart, TrendingDown, TriangleAlert, Users, Zap, } from 'lucide-react';
...
import { MiniStat, StatTile } from '../../components/ui/StatTile.tsx';
import { BucketPicker } from '../mfy/MfyDashboard.tsx';
import { useAlerts, useDebt, useDistrictOverview, useDistrictSeries, useDistance, useEfficiency, useEnergyBalance, useLossMap, useMfyRanking, useTechnicalLoss, useTpMonitoring, useWorks, } from '../../lib/queries.ts';
...
export default function DistrictDashboard() {
    ...
    if (overview.isError) {
        return (<>
        <PageHeader title={t('nav.dashboard')}/>
        <ErrorState message={overview.error instanceof Error ? overview.error.message : 'Noma’lum xatolik'} onRetry={() => void overview.refetch()}/>
      </>);
    }
    ...
    const urgent = alertItems.filter((a) => a.severity === 'critical' || a.severity === 'serious').length;
    const plannedWorks = (works.data ?? []).filter((w) => w.status !== 'COMPLETED');
    const completedWorks = (works.data ?? []).filter((w) => w.status === 'COMPLETED');
    return (<div className={overview.isFetching ? 'opacity-70 transition-opacity' : 'transition-opacity'}>
      <PageHeader actions={<>
            <PeriodPicker />
            <ReportMenu />
          </>} subtitle={t('app.subtitle')} title={t('nav.dashboard')}/>

      {/* ═══ 1-QATOR: 8 ta KPI kartasi ═══════════════════════════════════ */}
      <div className="mb-2.5 grid grid-cols-2 gap-2.5 md:grid-cols-4 xl:grid-cols-8">
        {tiles.map((tile) => (<StatTile key={tile.key} icon={TILE_ICONS[tile.key]} tile={tile} tone={TILE_TONES[tile.key] ?? 'blue'}/>))}
      </div>

      {/* ═══ 2-QATOR: yo'qotish xaritasi · balans · samaradorlik ═════════ */}
      <div className="mb-2.5 grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        <Panel className="xl:col-span-5" subtitle={t('panel.lossMapSub')} title={t('panel.lossMap')}>
          {lossMap.data && lossMap.data.length > 0 ? (<LossTreemap cells={lossMap.data} height={244} onSelect={goMfy}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel className="xl:col-span-4" title={t('panel.energyBalance')}>
          {balance.data && balance.data.length > 0 ? (<EnergyFlow height={244} nodes={balance.data}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel className="xl:col-span-3" title={t('panel.efficiency')}>
          {efficiency.data ? (<EfficiencyPanel data={efficiency.data}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>
      </div>
      ...
      <div className="mb-2.5 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <MiniStat hint={worst
            ...
            : '—'} icon={<TrendingDown className="size-4.5"/>} label="Eng yuqori yo‘qotish" tone="critical" value={worst ? pct(worst.lossPct, 2) : '—'}/>
        <MiniStat hint={`Jami ${num(technical.data?.length ?? 0)} ta mahalladan`} icon={<GaugeIcon className="size-4.5"/>} label="Standartdan chetlashgan" tone={overNorm.length > 0 ? 'warning' : 'good'} unit="ta" value={num(overNorm.length)}/>
        <MiniStat hint={`Jami ${num(tpTotalCounted)} ta TP · norma 300 m`} icon={<Ruler className="size-4.5"/>} label="Normadan uzoq TP" tone={tpOverDistance > 0 ? 'warning' : 'good'} unit="ta" value={num(tpOverDistance)}/>
        <MiniStat hint={`Jami ${num(alertItems.length)} ta ogohlantirish`} icon={<TriangleAlert className="size-4.5"/>} label="Shoshilinch chora" tone={urgent > 0 ? 'critical' : 'good'} unit="ta" value={num(urgent)}/>
      </div>

      {/* ═══ 4-QATOR: dinamika · ogohlantirish · qarzdorlik ══════════════ */}
      <div className="mb-2.5 grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        <Panel actions={<BucketPicker onChange={setBucket} value={bucket}/>} className="xl:col-span-5" title={t('panel.dynamics')}>
          {series.data && series.data.length > 0 ? (<TrendLine bucket={bucket} csvName="tuman-dinamika" height={214} points={series.data}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel className="xl:col-span-4" flush subtitle={t('panel.alertsSub')} title={t('panel.alerts')}>
          <AlertsPanel items={alertItems}/>
        </Panel>

        <Panel className="xl:col-span-3" title={t('panel.debt')}>
          {debt.data ? (<Donut centerLabel="jami qarzdorlik" centerValue={money(debt.data.totalMln).text} csvName="qarzdorlik" formatValue={(v) => money(v).text} height={190} slices={debt.data.byCategory.map((c) => ({
                id: c.category,
                label: c.labelUz,
                value: c.amountMln,
      ...
      <div className="mb-2.5 grid grid-cols-1 gap-2.5 lg:grid-cols-2 xl:grid-cols-12">
        <Panel actions={<Chip size="sm" variant="soft">
            ...
            </Chip>} className="xl:col-span-4" flush footerAction={{ label: 'Barcha transformatorlar', to: '/transformers' }} title={t('panel.tpMonitoring')}>
          <TpMonitorPanel rows={tp.data ?? []}/>
        </Panel>

        <Panel className="xl:col-span-4" flush footerAction={{ label: 'Barcha mahallalar', to: '/mahallalar' }} subtitle={t('panel.mfyRankingSub')} title={t('panel.mfyRanking')}>
          <MfyRankingPanel onSelect={goMfy} rows={ranking.data ?? []}/>
        </Panel>

        <Panel className="lg:col-span-2 xl:col-span-4" subtitle={t('panel.technicalLossSub')} title={t('panel.technicalLoss')}>
          {technical.data && technical.data.length > 0 ? (<DivergingBar height={272} onSelect={goMfy} rows={technical.data}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>
      </div>

      {/* ═══ 6-QATOR: masofa · TOP qarzdorlar · ishlar ═══════════════════ */}
      <div className="grid grid-cols-1 gap-2.5 lg:grid-cols-2 xl:grid-cols-12">
        <Panel className="xl:col-span-5" flush subtitle={t('panel.distanceSub')} title={t('panel.distance')}>
          <DistancePanel rows={distance.data ?? []} onSelect={goMfy}/>
        </Panel>

        <Panel className="xl:col-span-3" flush footerAction={{ label: 'Barcha qarzdorlar', to: '/debt' }} title={t('panel.topDebtors')}>
        ...
        <Panel actions={<Chip size="sm" variant="soft">
            ...
            </Chip>} className="lg:col-span-2 xl:col-span-4" flush footerAction={{ label: t('common.showAll'), to: '/works' }} title={t('panel.plannedWorks')}>
...
export function DistrictSummaryLine({ kwhIn, lossPct, debtMln, }) {
