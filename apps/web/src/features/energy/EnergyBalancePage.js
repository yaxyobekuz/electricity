...
import { useDistrictOverview, useDistrictSeries, useEnergyBalance, useLossMap, } from '../../lib/queries.ts';
...
export default function EnergyBalancePage() {
    ...
    return (<div className={overview.isFetching ? 'opacity-70 transition-opacity' : ''}>
      <PageHeader actions={<>
            <PeriodPicker />
            <ReportMenu />
          </>} subtitle="tarmoqqa kirgan energiya taqsimoti" title={t('nav.energyBalance')}/>

      {/* ── 4 ta xulosa ──────────────────────────────────────────────── */}
      <div className="mb-2.5 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <MiniStat hint={`${num(rows.length)} ta mahalladan`} icon={<Zap className="size-4.5"/>} label="Tarmoqqa kirgan" value={energy(totals?.kwhIn ?? 0).text}/>
        <MiniStat hint={inNode && soldNode ? `Ulushi: ${pct(soldNode.pct, 1)}` : ''} icon={<ShoppingCart className="size-4.5"/>} label="Sotilgan energiya" tone="good" value={energy(totals?.kwhSold ?? 0).text}/>
        <MiniStat hint={`Yo‘qotish darajasi: ${pct(totals?.lossPct ?? 0, 2)}`} icon={<TrendingDown className="size-4.5"/>} label="Jami yo‘qotish" tone="critical" value={energy(totals?.kwhLossTotal ?? 0).text}/>
        <MiniStat hint="tarmoqqa kirgan energiyaga nisbatan" icon={<ArrowRight className="size-4.5"/>} label="Foydali uzatish" tone="good" unit="%" value={totals && totals.kwhIn > 0
      ...
      <div className="mb-2.5 grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        <Panel className="xl:col-span-8" subtitle="manbadan iste’molchigacha bo‘lgan oqim" title="Energiya oqimi">
          {nodes.length > 0 ? (<EnergyFlow height={306} nodes={nodes}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel className="xl:col-span-4" flush title="Balans tarkibi">
          {nodes.length > 0 ? (<table className="dt">
              <thead>
                <tr>
                  <th>Modda</th>
              ...
              <tbody>
                {nodes.map((n) => (<tr key={n.key}>
                    <td className={n.key === 'in' ? 'font-semibold' : ''}>
                      {n.key !== 'in' && n.key !== 'sold' && (<span aria-hidden="true" className="mr-1 text-muted">└</span>)}
                      {n.labelUz}
                    </td>
            ...
            </table>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>
      </div>

      {/* ── Dinamika ─────────────────────────────────────────────────── */}
      <Panel actions={<ToggleButtonGroup aria-label="Guruhlash" selectedKeys={new Set([bucket])} selectionMode="single" size="sm" onSelectionChange={(keys) => {
                const next = [...keys][0];
                if (next === 'day' || next === 'week' || next === 'month')
                    setBucket(next);
            }}>
            {BUCKETS.map((b) => (<ToggleButton key={b.id} id={b.id}>
                {b.label}
              </ToggleButton>))}
          </ToggleButtonGroup>} className="mb-2.5" subtitle="oxirgi 90 kun" title="Balans dinamikasi">
        {series.data && series.data.length > 0 ? (<TrendLine csvName="balans-dinamika" height={252} points={series.data}/>) : (<EmptyPanel message={t('common.noData')}/>)}
      </Panel>

      {/* ── Mahallalar kesimi ────────────────────────────────────────── */}
      <Panel actions={<Chip size="sm" variant="soft">
            <Chip.Label>{num(rows.length)} ta mahalla</Chip.Label>
          </Chip>} flush title="Mahallalar kesimida">
        <div className="scroll-y max-h-125 overflow-x-auto">
          <table className="dt min-w-150">
            ...
            return (<tr key={r.mfyId} className="cursor-pointer" onClick={() => void navigate(`/dashboard/mfy/${r.mfyId}`)}>
