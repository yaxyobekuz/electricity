...
export default function DebtPage() {
    ...
    const perConsumer = totals && totals.consumersTotal > 0 ? (totalMln * 1e6) / totals.consumersTotal : 0;
    return (<div className={overview.isFetching ? 'opacity-70 transition-opacity' : ''}>
      <PageHeader actions={<>
            <PeriodPicker />
            <ReportMenu />
          </>} subtitle="toifalar va yirik qarzdorlar kesimida" title={t('nav.debt')}/>

      {/* ── Jami + toifalar ──────────────────────────────────────────── */}
      <div className="mb-2.5 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <MiniStat hint={`1 abonentga: ${num(perConsumer, 0)} so‘m`} icon={<CircleDollarSign className="size-4.5"/>} label="Jami qarzdorlik" tone="critical" value={money(totalMln).text}/>
        {byCategory.map((c) => (<MiniStat key={c.category} hint={`Ulushi: ${pct(c.pct, 1)}`} icon={CATEGORY_ICON[c.category]} label={CATEGORY_LABEL[c.category] ?? c.labelUz} tone={c.category === 'BUDGET' ? 'warning' : 'accent'} value={money(c.amountMln).text}/>))}
      </div>
      ...
      <div className="grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        <Panel className="xl:col-span-4" title="Toifalar bo‘yicha taqsimot">
          {byCategory.length > 0 ? (<Donut centerLabel="jami qarzdorlik" centerValue={money(totalMln).text} csvName="qarzdorlik-toifalar" formatValue={(v) => money(v).text} height={262} legendSide slices={byCategory.map((c) => ({
                id: c.category,
                label: c.labelUz,
                value: c.amountMln,
        ...
        <Panel actions={<Chip size="sm" variant="soft">
            ...
            </Chip>} className="xl:col-span-8" flush subtitle="eng yirik qarzdor tashkilotlar" title="Qarzdorlar ro‘yxati">
          {topDebtors.length > 0 ? (<div className="scroll-y max-h-125 overflow-x-auto">
              <table className="dt min-w-125">
                ...
                <tbody>
                  {topDebtors.map((d) => (<tr key={`${d.rank}-${d.debtorName}`}>
      ...
      <p className="mt-2.5 text-[10.5px] leading-relaxed text-muted">
        ...
        <button className="font-semibold text-accent hover:underline" type="button" onClick={() => void navigate('/passport')}>
          Pasportda ko‘rish
        </button>
      </p>
    </div>);
