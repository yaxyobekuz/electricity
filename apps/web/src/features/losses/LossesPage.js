...
import { useDistrictLossStructure, useDistrictOverview, useDistrictResults, useLossMap, useTechnicalLoss, } from '../../lib/queries.ts';
...
export default function LossesPage() {
    ...
    return (<div className={overview.isFetching ? 'opacity-70 transition-opacity' : ''}>
      <PageHeader actions={<>
            <PeriodPicker />
            <ReportMenu />
          </>} subtitle="tabiiy · texnik · noqonuniy foydalanish" title={t('nav.losses')}/>

      {/* ── 4 ta xulosa ──────────────────────────────────────────────── */}
      <div className="mb-2.5 grid grid-cols-2 gap-2.5 lg:grid-cols-4">
        <MiniStat hint={`Tarmoqqa kirgan: ${energy(totals?.kwhIn ?? 0).text}`} icon={<TrendingDown className="size-4.5"/>} label="Jami yo‘qotish" tone="critical" unit={pct(totals?.lossPct ?? 0, 2)} value={energy(totals?.kwhLossTotal ?? 0).text}/>
        <MiniStat hint={`Ulushi: ${pct(part('natural')?.pct ?? 0, 1)}`} icon={<Leaf className="size-4.5"/>} label="Tabiiy yo‘qotish" tone="good" value={energy(part('natural')?.kwh ?? 0).text}/>
        <MiniStat hint={`Ulushi: ${pct(part('technical')?.pct ?? 0, 1)}`} icon={<Activity className="size-4.5"/>} label="Texnik yo‘qotish" tone="warning" value={energy(part('technical')?.kwh ?? 0).text}/>
        <MiniStat hint={`Ulushi: ${pct(part('illegal')?.pct ?? 0, 1)}`} icon={<ShieldAlert className="size-4.5"/>} label="Noqonuniy foydalanish" tone="critical" value={energy(part('illegal')?.kwh ?? 0).text}/>
      </div>
      ...
      <div className="mb-2.5 grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        <Panel className="xl:col-span-3" title="Yo‘qotish tuzilmasi">
          {structure.data ? (<Donut centerLabel="jami yo‘qotish" centerValue={energy(structure.data.totalKwh).text} csvName="yoqotish-tuzilmasi" formatValue={(v) => energy(v).text} height={214} slices={parts.map((p) => ({
                id: p.key, label: p.labelUz, value: p.kwh, display: energy(p.kwh).text,
            }))}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel className="xl:col-span-6" subtitle="plitka o‘lchami — energiya oqimi, rangi — normadan farq" title="Yo‘qotish taqsimoti">
          {lossMap.data && lossMap.data.length > 0 ? (<LossTreemap cells={lossMap.data} height={266} onSelect={goMfy}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel className="xl:col-span-3" title="Natijadorlik">
          {results.data ? (<div className="flex flex-col gap-3">
              <div>
                <p className="text-[10.5px] leading-tight text-muted">
                  Yo‘qotish darajasi ({results.data.periodFrom} → {results.data.periodTo})
                </p>
                <p className="mt-1.5 flex items-center gap-2">
                  <span className="text-[21px] font-bold leading-none" style={{ fontVariantNumeric: 'tabular-nums' }}>
                    {pct(results.data.lossPctStart ?? 0, 1)}
                  </span>
                  <TrendingDown aria-hidden="true" className="size-4 text-muted"/>
                  <span className="text-[21px] font-bold leading-none" style={{ fontVariantNumeric: 'tabular-nums', color: 'var(--viz-good)' }}>
                    {pct(results.data.lossPctEnd ?? 0, 1)}
                  </span>
                </p>
              </div>

              {results.data.improvementPp !== null && (<p className="text-[12px] font-semibold" style={{
              ...
              <div className="border-t border-separator pt-2.5">
                ...
                <p className="mt-1 text-[17px] font-bold leading-tight" style={{ fontVariantNumeric: 'tabular-nums', color: 'var(--viz-good)' }}>
              ...
              <div className="border-t border-separator pt-2.5">
                ...
                <p className="mt-1 text-[17px] font-bold leading-tight" style={{
      ...
      <div className="grid grid-cols-1 gap-2.5 xl:grid-cols-12">
        <Panel className="xl:col-span-5" subtitle="amaldagi va standart texnik yo‘qotish farqi" title="Standartdan chetlashish">
          {technical.data && technical.data.length > 0 ? (<DivergingBar height={430} onSelect={goMfy} rows={technical.data}/>) : (<EmptyPanel message={t('common.noData')}/>)}
        </Panel>

        <Panel actions={<Chip size="sm" variant="soft">
              <Chip.Label>{num(ranked.length)} ta mahalla</Chip.Label>
            </Chip>} className="xl:col-span-7" flush footerAction={{ label: 'Barcha mahallalar', to: '/mahallalar' }} title="Mahallalar bo‘yicha">
          <div className="scroll-y max-h-115 overflow-x-auto">
            <table className="dt min-w-125">
              ...
              <tbody>
                {ranked.map((r, i) => (<tr key={r.mfyId} className="cursor-pointer" onClick={() => goMfy(r.mfyId)}>
                    ...
                    <td className="num whitespace-nowrap font-medium" style={{ color: r.gapPp > 0 ? 'var(--viz-critical)' : 'var(--viz-good)' }}>
                      {r.gapPp > 0 ? '+' : ''}
                      {r.gapPp.toFixed(2)} p.p.
                    </td>
                    <td>
                      <Chip className="whitespace-nowrap" color={r.status === 'good' ? 'success'
                : r.status === 'warning' ? 'warning' : 'danger'} size="sm" variant="soft">
