...
export function PassportTable({ passport, compact }) {
    const cyr = currentScript() === 'cyrl';
    return (<table className={cn('dt w-full', compact && 'text-xs')}>
      <thead>
        <tr>
      ...
      <tbody>
        ...
        */}
        {passport.rows.map((row) => (<Fragment key={row.no}>
            <tr className="font-medium">
              ...
              <td className="no-print">
                <Chip color={row.source === 'input' ? 'accent' : 'default'} size="sm" variant="soft">
                  <Chip.Label>
            ...
            {row.children?.map((child, ci) => (<tr key={`${row.no}-${ci}`} className="text-muted">
                <td />
