...
export default function PassportPage() {
  ...
  const [selectedMfy, setSelectedMfy] = useState<number | null>(routeMfyId);
  ...
  if (tuman.isError) {
    return (
      <ErrorState
        message={tuman.error instanceof Error ? tuman.error.message : 'Ma’lumot topilmadi'}
        onRetry={() => void tuman.refetch()}
      />
    );
  }
        ...
        >
          <Tabs.ListContainer>
            <Tabs.List aria-label="Pasport ko‘rinishi">
              ...
              <Tabs.Tab id="reconcile">
                ...
                {mismatches.length > 0 && (
                  <Chip className="ml-1.5" color="danger" size="sm" variant="soft">
          ...
          <Tabs.Panel className="p-4" id="tuman">
            {tuman.data ? (
              <>
                <PassportHeader
                  frozen={tuman.data.frozen}
                  period={tuman.data.period}
                  scopeName={tuman.data.scopeName}
                />
          ...
          <Tabs.Panel className="p-4" id="mfy">
            <div className="mb-3">
              <Select
                ...
                value={selectedMfy === null ? null : String(selectedMfy)}
                onChange={(v) => setSelectedMfy(v === null ? null : Number(v))}
              >
                <Select.Trigger>
                  <Select.Value />
                  <Select.Indicator />
            ...
            {selectedMfy === null ? (
              <EmptyPanel message="Pasportni ko‘rish uchun mahallani tanlang" />
            ) : mfy.isLoading ? (
              <LoadingState rows={3} />
            ) : mfy.data ? (
              <>
                <PassportHeader
                  frozen={mfy.data.frozen}
                  period={mfy.data.period}
                  scopeName={mfy.data.scopeName}
                />
                <div className="scroll-y max-h-[70vh] rounded-lg border border-border/70">
                  ...
                  <tbody>
                    {(reconcile.data ?? []).map((r) => (
                      <tr key={r.no}>
                        ...
                        <td>
                          {r.ok ? (
                            <span className="inline-flex items-center gap-1 text-[11px] text-viz-good">
                              <CheckCircle2 className="size-3.5" /> Mos
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1 text-[11px] text-viz-critical">
